...
link:https://drill.apache.org/?ref=rittmanmead.com[Apache Drill] is an engine that can connect to many different data sources, and provide a SQL interface to them. It's not just a wanna-be SQL interface that trips over at anything complex - it's a link:https://drill.apache.org/docs/sql-reference-introduction/?ref=rittmanmead.com[hugely functional one] including support for many built in functions as well as link:https://drill.apache.org/docs/sql-window-functions-introduction/?ref=rittmanmead.com[windowing functions]. Whilst it can connect to standard data sources that you'd be able to query with SQL anyway, like Oracle or MySQL, it can also work with flat files such as CSV or JSON, as well as Avro and Parquet formats. It's this capability to run SQL against files that first piqued my interest in Apache Drill. I've been spending a lot of time looking at Big Data architectures and tools, including link:https://speakerdeck.com/rmoff/unlock-the-value-in-your-big-data-reservoir-using-oracle-big-data-discovery-and-oracle-big-data-spatial-and-graph?ref=rittmanmead.com[Big Data Discovery]. As part of this, and experimenting with link:http://www.rittmanmead.com/2016/07/stream-analytics-processing-kafka-oracle-stream-analytics/?ref=rittmanmead.com[data pipeline options] one of the gaps that I've found is the functionality to dig through files in their raw state, before they've been brought into something like Hive which would enable their exploration through BDD and other tools.
...
It's very simple to get going with Apache Drill - just link:https://drill.apache.org/download/?ref=rittmanmead.com[download] and unpack it, and run. Whilst it can run distributed across machines for performance, it can also run standalone on a laptop.
...
If you get `No current connection` or `com.fasterxml.jackson.databind.JavaType.isReferenceType()Z` then you have a link:http://rmoff.net/2016/06/20/apache-drill-not-connected/?ref=rittmanmead.com[conflicting JAR problem] (e.g. I encountered this on Oracle's BigDataLite VM), and should launch it with a clean environment
...
On the Apache Drill website there's some useful link:https://drill.apache.org/docs/tutorials/?ref=rittmanmead.com[tutorials], including one using data link:http://www.yelp.com/dataset_challenge?ref=rittmanmead.com[provided by Yelp]. This was the dataset that originally got me looking at Drill, since I was using it as an input to link:https://speakerdeck.com/rmoff/unlock-the-value-in-your-big-data-reservoir-using-oracle-big-data-discovery-and-oracle-big-data-spatial-and-graph?ref=rittmanmead.com[Big Data Discovery] (BDD) but struggling on two counts. First up was how best to define a suitable Hive table over it in order to ingest it to BDD. Following from this was trying to understand what value there might be in the data which would drive how long to spend perfecting the way in which I exposed the data in Hive. The examples below show the kind of complications that complex JSON can introduce when queried in a tabular fashion.
...
You'll notice in the above output that the `full_address` field has line breaks in -- we can just use a link:https://drill.apache.org/docs/about-sql-function-examples/?ref=rittmanmead.com[SQL Function] to replace line breaks with commas:
...
Here's an example of using Drill to query a local file holding some Twitter data. You can link:https://gist.github.com/b81b40df61d3faa1f4bf05193cfeec6d?ref=rittmanmead.com[download the file here] if you want to try querying it yourself.
...
That's the complete data in the file - so Drill is right - the JSON is corrupted. If we drop that last record and create a new file (link:https://gist.github.com/9edca5e444143a00a8207ed41a364b39?ref=rittmanmead.com[`geo_tweets.fixed.json`])
...
This can be made more readable by using link:https://drill.apache.org/docs/with-clause/?ref=rittmanmead.com[Common Table Expressions] (CTE, also known as subquery factoring) for the same result:
...
JSON files are relatively easy to interpret because they have a semi-defined schema within them, including column names. CSV (and character delimited files in general), on the other hand, are a bit more of a 'wild west' when it comes to reliably inferring column names. You can link:https://drill.apache.org/docs/text-files-csv-tsv-psv/?ref=rittmanmead.com[configure Apache Drill] to ignore the first line of a CSV file (on the assumption that it's a header) if you want to, or to take them as column names. If you don't do this and query a CSV file that looks like this:
...
+
* Configuring additional storage plugins (e.g. link:https://drill.apache.org/docs/rdbms-storage-plugin/?ref=rittmanmead.com[database], link:https://drill.apache.org/docs/file-system-storage-plugin/?ref=rittmanmead.com[hdfs], etc)
...
Per link:https://drill.apache.org/docs/rdbms-storage-plugin/?ref=rittmanmead.com[the documentation] it's easy to query data residing in a RDBMS, such as Oracle. Simply copy the JDBC driver into Apache Drill's jar folder:
...
Just as Oracle has its Cost Based Optimiser (CBO) which helps it determine how to execute a query, and do so most efficiently, Apache Drill has an execution engine that determines how to actually execute the query you give it. This also includes how to split it up over multiple nodes ("drillbits") if available, as well as optimisations such as link:https://drill.apache.org/docs/how-to-partition-data/?ref=rittmanmead.com[partition pruning] in certain cases. You can read more about link:https://drill.apache.org/docs/drill-query-execution/?ref=rittmanmead.com[how the query execution works here], and watch a link:https://www.youtube.com/watch?v=0rurIzOkTIg&ref=rittmanmead.com[good explanation of it] here. To view the explain plan for a query use `explain plan` :
...
The MapR Drill ODBC driver comes with a tool called *link:https://drill.apache.org/docs/drill-explorer-introduction/?ref=rittmanmead.com[Drill Explorer]*. This is a GUI that enables you to explore the data by navigating the databases (==storage plugins) and folders/files within, previewing the data and even creating views on it.
