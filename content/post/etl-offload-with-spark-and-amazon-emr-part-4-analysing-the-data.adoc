...
One of the things I wanted to explore during this project was the feasibility and response times that "SQL on Hadoop" engines could bring. link:https://hive.apache.org/?ref=rittmanmead.com[Hive] is probably the most well known of these, with other options including link:http://impala.apache.org/?ref=rittmanmead.com[Apache Impala (incubating)], link:https://drill.apache.org/?ref=rittmanmead.com[Apache Drill], link:https://prestodb.io/?ref=rittmanmead.com[Presto], and even link:https://www.oracle.com/database/big-data-sql/index.html?ref=rittmanmead.com[Oracle's Big Data SQL]. All these tools read data that is _not stored in a proprietory database format_ but stored in an open format, such a simple text file, on open storage platform such as HDFS. More commonly, formats (also open, non-proprietory) which are optimised for performance such as Parquet or ORC are used.
...
For an overview of SQL-on-Hadoop engines see link:https://speakerdeck.com/gregrahn/oaktable-world-2014-the-current-state-of-sql-plus-hadoop?ref=rittmanmead.com[this presentation from Greg Rahn]. It's a couple of years old but pretty much still current bar the odd version and feature change.


== Redshift


link:https://aws.amazon.com/redshift?ref=rittmanmead.com[Redshift] is not SQL-on-hadoop - it is a full-blown database. Specifically, it is a proprietory implementation of Postgres by Amazon, running as a service on their cloud. Just as you can provision an EMR cluster of any required size, you can do the same for Redshift. As your capacity and processing requirements change, you can scale your Redshift cluster up and down.
...
To work with Redshift you can use a tool just as link:http://docs.aws.amazon.com/redshift/latest/mgmt/connecting-using-workbench.html?ref=rittmanmead.com[SQL WorkbenchJ], or the `psql` commandline tool. I installed the latter on my Mac using link:http://brew.sh/?ref=rittmanmead.com[brew] and `brew install postgres`.
...
The version of Hive that I was using on EMR was automagically configured to link:https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez?ref=rittmanmead.com[use Tez] as its execution engine, instead of the traditional map/reduce of the original Hadoop platform.
...
although it's not clear if that's still true in the context of the latest link:https://blog.cloudera.com/blog/2016/08/analytics-and-bi-on-amazon-s3-with-apache-impala-incubating/?ref=rittmanmead.com[Impala S3 optimisations].


== Presto


Presto is an open source project that originated at Facebook. Similar to Apache Drill (below), it can query across data (and federate the results) from link:https://prestodb.io/docs/current/connector.html?ref=rittmanmead.com[multiple sources] including Hive (and thus S3), MongoDB, MySQL, and even Kafka.
...
A final note on performance - link:http://tech.marksblogg.com/billion-nyc-taxi-rides-s3-vs-hdfs.html?ref=rittmanmead.com[this blog] does a comparison of Presto querying data held in S3 vs HDFS on EMR. HDFS on EMR is quicker, generally about 1.5 times or so - but you of course need your data loaded into HDFS on a running EMR cluster, whereas S3 is there on demand whenever you want it.


== Drill


Apache Drill is another open-source tool, similar in concept to Presto, in that it enables querying across data held in multiple sources. I've written about it previously link:https://www.mapr.com/blog/how-guide-getting-started-apache-drill?ref=rittmanmead.com[here] and link:https://www.mapr.com/blog/essential-tips-using-apache-drill-obiee-12c?ref=rittmanmead.com[here]. Whilst EMR has an option to provision a Drill cluster as part of an EMR build, it didn't seem to work when I tried it - and with Presto running I didn't spend the time digging into Drill. Given another time and project though, I'd definitely be looking to run it against this kind of data to see how it handled it. A link:https://lists.apache.org/thread.html/%3CEA6DA00B-9249-4D31-9C8C-AFFC44570F98@xhochy.com%3E?ref=rittmanmead.com[recent thread on the Drill mailing list] gave some interesting information on performance.


== Athena


Amazon's link:https://aws.amazon.com/athena/?ref=rittmanmead.com[Athena] tool was link:https://aws.amazon.com/about-aws/whats-new/2016/11/introducing-amazon-athena-a-pay-as-you-go-interactive-query-service-that-makes-it-easy-to-analyze-data-in-amazon-s3-using-standard-sql/?ref=rittmanmead.com[announced] at link:https://reinvent.awsevents.com/?ref=rittmanmead.com[re:Invent 2016]. Even though it was made GA after the client project being discussed here and therefore not evaluated, it is definitely worth mentioning. It provides "serverless" SQL querying of data held in S3. Under the covers it uses Presto (which is one of the tools I evaluated above). The benefit of Athena is that you wouldn't need to provision and configure actual servers to use Presto. You work with it through the web-based interface, or link:https://docs.aws.amazon.com/athena/latest/ug/connect-with-jdbc.html?ref=rittmanmead.com[JDBC]. This is a pretty big point to make - *you can query your data, held in an open format, on demand, using SQL, without having to move it into a database or build a server to run a query engine*.


Athena looks interesting, but one of the main things that struck me about it was that it is _not_ something you would simply point at piles of data on S3 and build your analytics systems on. The link:https://aws.amazon.com/athena/pricing/?ref=rittmanmead.com[cost] is _per query_, and is currently link:https://aws.amazon.com/athena/pricing/?ref=rittmanmead.com[$5 per TB scanned]. FIVE DOLLARS, per terabyte of data _SCANNED_. Not retrieved. Scanned. So in order to not run up big AWS bills if you've got lots of data, you're going to need to do smart things to reduce the size of data scanned. Partitioning your data, and compressing it, will both help. As it happens, these are the things that are going to increase performance too, so it's not wasted effort. There's link:http://tech.marksblogg.com/billion-nyc-taxi-rides-aws-athena.html?ref=rittmanmead.com[a good writeup here] demonstrating Athena, and the difference that using an appropriate storage format for the data makes to performance and volumes of data scanned (and thus cost).
...
You can read more about Athena link:https://aws.amazon.com/blogs/big-data/analyzing-data-in-s3-using-amazon-athena/?ref=rittmanmead.com[here].
