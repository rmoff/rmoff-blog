...
Would we as developers use a stone when we needed a hammer to bang in a nail? No, that would be stupid—we use the right tool for the job, ofc. Hammers are an evolution of the tool from a crude stone, and we use that because it's the best tool for the job.
But once the mechanical hammer drill came along, do we cling to our manual hammer when we've got a nail to bang into a brick wall? Again, no, that would be stupid. We want to use the *best tool for the job*.
...
If you, as a senior+ developer, think of an LLMs as a very eager junior developer working for you.
They're fresh eyed and bushy tailed, and goddamnit they talk too much, doesn't listen enough, and makes stupid mistakes.
...
It will never rolls its eyes and sigh at you when you ask for the hundredth amendment to your original spec, so it's easy to get sucked into endless fiddling and tweaking.

I found I quickly burnt through my Pro token allowance, which actually served well as a gatekeeper on my time, forcing me to step back until the token were refreshed.
...
image::/images/2026/01/2026-01-27T15-36-05-008Z.png[]
...
If you're doing any kind of webdev work, following https://blog.jenkster.com/2026/01/making-your-agent-see-what-its-missing/[Kris Jenkins' tip] and use https://playwright.dev/docs/intro#installing-playwright[Playwright] so that Claude can "see" as it develops.
...
TIP: You might _not_ want to always do this; see my observation above about context switching and continuous interuptions
...
image::/images/2026/01/2026-01-27T15-45-53-209Z.png[]

Also checkout https://ccusage.com/[`ccusage`] which uses the Claude log data to calculate usage and break it down in different ways which can help you optimise your use of it
...
* `CLAUDE.md`—where Claude makes a note of what it is your building and core principles, toolsets, etc
...
I've been experimenting with a couple of non-coding examples, both pairing Claude with https://github.com/basicmachines-co/basic-memory[basic-memory] and an Obsidian vault.

* Planning a holiday.
Iteratively building up with CLAUDE.md that captures the requirements of the holiday, it can then help with itineraries, checklists, discuss areas, etc etc.
...
_Obviously_ I am not following it blindly but as an exercise (geddit?!) in integration and LLMs, it's pretty fun.
