...
categories:
- Apache Flink
image: "/images/2024/08/flinksql-s3.jpg"
---
...
Well perhaps it is.
I’m not yet sure.
But what’s led me down this path again is trying to get the Delta Lake connector to work in Flink SQL, and butting up against S3 problems.
...
_For Delta Lake and Flink itself you’ll need to stay tuned to the blog, as I write about that in a separate article._
_Today we’re just looking at some troubleshooting tips for S3 access in Flink SQL in general._

So what we’ve got here is a methodical explanation of how I understand the S3 configuration and libraries to work, keeping things as vanilla as possible to start with.
Filesystem connector, JSON format.

_Huge caveat: I am just a humble end-user of this stuff._
_I don’t code Java._
_I don’t actually know what I’m doing half the time 😉._
...
----
_I’m using json format not because it’s a good idea, but because it doesn’t need any further dependencies installed, and I want to keep this as small and easily-reproduced as possible._

I’m using the default ephemeral in-memory catalog, so each time I restart my session I have to define the table again.

Once the table is created I run the `INSERT`, which the SQL Client passes as a job to the Job Manager to run (`Submitting SQL update statement to the cluster…`).
This means that the SQL Client will say `successfully` in the message after running the `INSERT` but if you look carefully it’s just saying it’s _submitted_ successfully:
...
If you don’t include this and try to write to an `s3://` path you’ll get this error:
...
----
For an `s3a://` path you’ll get basically the same:
...
[…]
----
So per  link:https://nightlies.apache.org/flink/flink-docs-stable/docs/deployment/filesystems/plugins/[the docs] , you need the Hadoop S3 plugin, which ships with Flink but isn’t in place by default.
...
=== Log File Diving
With the log levels set to `TRACE`, let’s go and have a look at what we get.
...
The Hadoop `S3AFileSystem` is shown as `Initializing` for `warehouse`, where `warehouse` is the name of the bucket that’s been specified for writing the table data to (`'path' = 's3a://warehouse/t_foo_fs/'`).
...
Then some more config values, and then an interesting one (since we’re using MinIO)—the endpoint configuration.
Since we’ve not yet set it, it’s unsurprising to see that it’s using the default (`Using default endpoint`):
...
----
I’ll skip over a bit more of the same kind of background `DEBUG` stuff, and highlight this bit where we start to see the file paths mentioned.
...
----
Remembering the value of `path ('path' = 's3a://warehouse/t_foo_fs/')` we can see the bucket has been stripped away to give us just the 'folder' (which isn’t, on S3) of `t_foo_fs`, and then the actual data for Flink to upload (`part-5d498f53-ec06-4ee4-9fe2-5c7763755200-0-0`).

So we’re now at the part of the S3 process where it wants to write the data.
We kinda know it’s going to fail anyway because we didn’t configure the endpoint; but we also didn’t configure any credentials and it’s going to be that which trips things up first:
...
----
What’s useful here is you can see the code go through the different credential source options, including environment variables (`EnvironmentVariableCredentialsProvider`) and config file (`SimpleAWSCredentialsProvider`).
...
So let’s rectify that and tell Flink how to authorise to S3 (MinIO).
...
Which since we’re using the credentials for MinIO, isn’t going to work.
...
----
There’s then a second HTTP request (a `POST`) for the file itself:
...
If you’re using an S3-compatible object store, such as MinIO, you need to tell the Flink S3 client where to find it, since as we saw above it defaults to literally `warehouse.s3.amazonaws.com`.

Configuring the endpoint is covered  link:https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/s3/#configure-non-s3-endpoint[clearly in the docs] —add it to your Flink config:
...
So, with the endpoint set, let’s see what happens.
We’ll pick up where it went wrong last time; the HTTP calls to the S3 endpoint which should now be correct:
...
----
Somehow it’s getting `warehouse.localhost` from our configuration, which is not a hostname that exists from my machine.
...
It’s also valid to specify `fs.s3a.` directly—they get read and mapped the same:
...
The code mentions a `flink.hadoop.` prefix but this seems to be  link:https://github.com/apache/flink/blob/master/flink-filesystems/flink-s3-fs-hadoop/src/main/java/org/apache/flink/fs/s3hadoop/S3FileSystemFactory.java#L41-L47[overridden for flink-s3-fs-hadoop]  as a set of `FLINK_CONFIG_PREFIXES` which can be `s3., s3a., or fs.s3a.`—they’re all the same.
...
There is, and it’s called  link:https://www.decodable.co/product[Decodable]  😀

With pre-built connectors for S3, Apache Iceberg, Delta Lake, and dozens more, it’s the easiest way to move data.
To use a connector you simply provide the necessary configuration—there’s not a JAR in sight!

Decodable also has managed Flink, so if you _really_ want to write this stuff by hand, you can, and we’ll run it for you.
