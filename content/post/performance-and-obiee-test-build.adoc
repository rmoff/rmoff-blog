...
link:http://jmeter.apache.org/?ref=rittmanmead.com[JMeter] is open-source software maintained by the Apache Software Foundation. It is a flexible and powerful tool that has a GUI design interface that can run on any java-supporting desktop (e.g. Windows/Linux/Mac). The test execution can be run from the GUI, or “headless” from the commandline.


To get started with JMeter, simply link:http://jmeter.apache.org/download_jmeter.cgi?ref=rittmanmead.com[download it], and uncompress the archive (tgz / zip). Go to the `bin` folder and doubleclick on jmeter.sh or jmeter.bat. This will launch JMeter.
...
+
* The link:https://code.google.com/p/jmeter-plugins/?ref=rittmanmead.com[JMeter Plugins] are very useful for additional graphing options and better user control (eg *jp@gc - Stepping Thread Group*)
...
+
* A sample OBIEE JMeter jmx script can be link:https://gist.github.com/rmoff/5161818?ref=rittmanmead.com[downloaded from here]
...
To use nqcmd, you first need to set up the OBI environment variables. On Linux/Unix this is done by “link:http://stackoverflow.com/questions/5030090/why-use-a-dot-to-execute-the-profile?ref=rittmanmead.com[dot-sourcing]” a file which sets the environment for the current shell. The first character to type is dot, then space, then the path to the `bi-init.sh` script:
...
Add a listener to visualise the results, such as *jp@gc - Response Times Over Time* (part of link:https://code.google.com/p/jmeter-plugins/?ref=rittmanmead.com[JMeter Plugins])
...
link:https://gist.github.com/rmoff/5162390?ref=rittmanmead.com[Download the JMeter JMX script here].
