...
┌──────────────────────┬───────────────┬───────────────┬──────────────┬────────────────────────────────────────────────────────────────────────────────────────────────────┐
│       click_ts       │    ad_cost    │ is_conversion │   user_id    │                                                _cdc                                                │
│ timestamp with tim…  │ decimal(38,2) │    boolean    │   varchar    │  struct(op varchar, ts timestamp with time zone, "offset" bigint, source varchar, target varchar)  │
├──────────────────────┼───────────────┼───────────────┼──────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ 2025-07-03 15:30:0…  │          2.50 │ true          │ user_12345   │ {'op': I, 'ts': '2025-07-03 10:32:05.717+01', 'offset': 1, 'source': public.clicks, 'target': pu…  │
│ 2023-02-01 13:30:2…  │          1.50 │ true          │ 001234567890 │ {'op': I, 'ts': '2025-07-03 10:29:29.777+01', 'offset': 0, 'source': public.clicks, 'target': pu…  │
└──────────────────────┴───────────────┴───────────────┴──────────────┴────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
A quick peruse of https://iceberg.apache.org/docs/nightly/kafka-connect/#configuration[the docs] points us at `iceberg.tables.evolve-schema-enabled`, which is `false` by default.
...
│ user_id       │ VARCHAR                                                                                          │ YES     │ NULL    │ NULL    │ NULL    │
│ _cdc          │ STRUCT(op VARCHAR, ts TIMESTAMP WITH TIME ZONE, "offset" BIGINT, source VARCHAR, target VARCHAR) │ YES     │ NULL    │ NULL    │ NULL    │
│ campaign_id   │ VARCHAR                                                                                          │ YES     │ NULL    │ NULL    │ NULL    │
└───────────────┴──────────────────────────────────────────────────────────────────────────────────────────────────┴─────────┴─────────┴─────────┴─────────┘
----
...
│ 2025-07-03 15:30:00+01   │          2.50 │ true          │ user_12345   │ campaign_summer_2025 │
└──────────────────────────┴───────────────┴───────────────┴──────────────┴──────────────────────┘
----

== Sending data from multiple topics to many Iceberg tables
...
I'll create a Debezium connector that's going to pick up all of them, writing each to its own Kafka topic:
...
Fortunately we still have a table as part of that data, as we can see from a snippet from earlier this article
...
[…]]┬──────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
[…]]│                                                     _cdc                                                     │
[…]]│       struct(op varchar, ts timestamp with time zone, "offset" bigint, source varchar, target varchar)       │
[…]]┼──────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
[…]]│ {'op': I, 'ts': '2025-07-02 10:08:35.608+01', 'offset': 0, 'source': public.clicks, 'target': public.clicks} │
----
...
It's just not happy with it, because it's taken the schema from Postgres (`public` in the query output above, and `europe` in our example here) as the Iceberg _database_.
...
But what if we are sending data from Kafka topics that _haven't_ been populated by Debezium?
...
[source,bash]
----
$ docker compose exec -it kcat kcat -b broker:9092 -C -t orders_json -c1
----
...
But what about if we've gone more topics like this, perhaps `products_json` too?

[source,bash]
----
$ docker compose exec -it kcat kcat -b broker:9092 -C -t products_json -c1
----

[source,javascript]
----
{"product_id": "prod_001", "name": "Gaming Laptop", "category": "Electronics", "price": 1299.99, "stock": 15}
----
...
But how to route them sensibly to an Iceberg table based on their _topic name, rather than a field within the payload itself which is what the Iceberg sink's dynamic routing is based on.
...
tl;dr I hadn't escaped the `$` of the `$0` in my config, meaning that `$0` was interpretted as a https://www.gnu.org/software/bash/manual/bash.html#Special-Parameters[special shell parameter] and replaced with `-zsh` _when it was passed to kcctl_.
...
I've https://github.com/apache/iceberg/issues/13457[logged this as a bug] that seems to be related to the use of SMTs to populate the field used by `iceberg.tables.dynamic-enabled` / `iceberg.tables.route-field`.
