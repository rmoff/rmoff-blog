...
Kafka has been designed from the outset to be distributed and fault-tolerant, and for very high performance (low latency) too. For a good introduction to Kafka and its concepts, the link:http://kafka.apache.org/documentation.html#introduction[introduction] section of the documentation is a good place to start, as is Gwen Shapira’s link:http://www.slideshare.net/gwenshap/kafka-for-dbas[Kafka for DBAs] presentation.


If you’re interested in reading more about Kafka, the article that really caught my imagination with its possibilities was by link:http://www.confluent.io/blog/apache-kafka-samza-and-the-unix-philosophy-of-distributed-data[Martin Kleppmann] in which he likens (broadly) Kafka to the unix Pipe concept, being the joiner between components that never had to be designed to talk to each other specifically.
