...
_Sometimes it's not possible to have too much of a good thing, and whilst this blog may look at first-glance rather similar to the one that_ link:https://www.decodable.co/blog/kafka-to-iceberg-with-decodable[I published just recently] _, today we're looking at a 100% pure Apache solution._
_Because who knows, maybe you prefer rolling your own tech stacks instead of letting Decodable do it for you 😉._
...
Supported by most of the big platforms—including now, notably, Databricks with their acquisition of Tabular—it's proving popular in "Data Lakehouse" implementations.
...
Depending on your pipeline infrastructure of choice you might decide to do this using link:https://github.com/tabular-io/iceberg-kafka-connect[Kafka Connect] , or as is the case commonly these days, Apache Flink—and that's what we're going to look at here.
...
----
All that's left to do now is create the Iceberg table, using the `CREATE TABLE…AS SELECT` syntax:
...
We’re writing the Iceberg files to MinIO, which is a self-hosted S3-compatible object store.
...
* I presented my working Iceberg deployment as a _fait accompli_, when in reality it was the output of rather a lot of learning and link:https://libquotes.com/linus-torvalds/quote/lbr1k4j[random jiggling]  until I got it to work—so I'm going to explain that bit too.
...
As I explored (_at length_ 😅) in link:https://www.decodable.co/blog/flink-sql-and-the-joy-of-jars[several]  link:https://www.decodable.co/blog/flink-sql-misconfiguration-misunderstanding-and-mishaps[previous]  link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[articles] , you often need to add dependencies to Flink for it to work with other technologies.
...
Of course, if you don't enjoy _that_ kind of fun, you could just link:https://www.decodable.co/blog/kafka-to-iceberg-with-decodable[let Decodable do it for you] .


==== Catalog
To use Iceberg you need to have a catalog metastore.
Catalogs in Flink can be a bit hairy to understand at first, but have a read of link:https://www.decodable.co/blog/catalogs-in-flink-sql-a-primer[this primer]  that I wrote for a gentle yet thorough introduction to them.

Iceberg can work with different catalogs (I explore several of them link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[here] ), and in this blog article I've used the Hive Metastore (HMS).
...
To use the Hive metastore with Flink you need the link:https://nightlies.apache.org/flink/flink-docs-master/docs/connectors/table/hive/overview/#dependencies[Hive connector]  `flink-sql-connector-hive-3.1.3` which link:https://www.decodable.co/blog/flink-sql-and-the-joy-of-jars[you can find]  on link:https://repo1.maven.org/maven2/org/apache/flink/flink-sql-connector-hive-3.1.3_2.12/[Maven repository] —make sure you use the correct one for your version of Flink.

As well as the Hive connector, you need Hadoop dependencies.
I wrote about this link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[elsewhere]  but you either install the full Hadoop distribution, or cherry-pick just the JARs needed.
...
==== Want the easy route?
_(Of course, the_*really*_easy route is to_ link:https://www.decodable.co/blog/kafka-to-iceberg-with-decodable[use Decodable] _, where we do all of this for you._
_😁)_
...
* link:https://github.com/decodableco/examples/tree/main/kafka-iceberg/apache-flink[and more] …sample SQL statements, step-by-step README, and some other noodling around with Iceberg and catalogs for good measure.
...
You can find out more here about link:https://www.decodable.co/blog/flink-sql-and-the-joy-of-jars[locating the correct JAR to download]  and where to put it once you've downloaded it.
...
_This is a really fun one—I got it when I’d not configured_`hive.metastore.local`
_in Flink’s_`conf/hive-site.xml`
...
The catalog I used was Hive Metastore, but there are others—such as AWS Glue.