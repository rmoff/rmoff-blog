...
Since the root data element (`db`) is an array, we need to link:https://drill.apache.org/docs/flatten/[`FLATTEN`] it:
...
Using `jq` again we can see the structure of the JSON file, with the code taken link:https://github.com/stedolan/jq/issues/243#issuecomment-45460474[from here]:
...
all with a single tool that can run on a laptop or desktop, and link:https://drill.apache.org/docs/interfaces-introduction/[supports ODBC and JDBC] for use with your favourite BI tools. For data exploration and understanding new datasets, Apache Drill really does rock!
