...
dbt is a tool that arrived on the data engineering scene around 2020.
...
In 2022 I made a link:/2022/10/20/data-engineering-in-2022-exploring-dbt-with-duckdb/[couple] of /2022/10/24/data-engineering-in-2022-wrangling-the-feedback-data-from-current-22-with-dbt/[attempts] to learn dbt, but it never really 'clicked'.

I'm pleased to say that as of today, dbt has definitely 'clicked'.
How do I know?
Because not only can I explain what I've built, but I've even had the lightbulb-above-the-head moment seeing it in action and how elegant the code used to build pipelines with dbt can be.

You can find the full project on https://github.com/rmoff/env-agency-dbt/[GitHub here].
...
I've used this same set of data many times before, because it provides just the right balance of real-world imperfections, interesting stories to discover, data modelling potential, and enough volume to be useful but not too much to overwhelm.
...
This is where we remember exactly what dbt is—and isn't—for.
Whilst DuckDB can pull data from an API directly, it doesn't map directly to capabilities in dbt for a good reason—dbt is for *transforming* data.

That's not to say dbt is nothing but flexible, and its ability to run https://docs.getdbt.com/docs/build/jinja-macros[Jinja-based macros] gives it superpowers for bending to most wills.
Here's how we'll pull in the readings API data:

[source,sql]
----
{% macro load_raw_readings() %}
{% set endpoint = var('api_base_url') ~ '/data/readings?latest' %} <3>
...
{% endmacro %}
----
<1> As it happens, we *are* using DuckDB's `read_json` to fetch the API data (contrary, much?)
<2> Disassmble the REST payload to get the most recent timestamp of the data, store it as its own column for freshness tests later
<3> Variables are defined in https://github.com/rmoff/env-agency-dbt/blob/master/dbt_project.yml#L38[`dbt_project.yml`]

Even though we are using DuckDB for the extract phase of our pipeline, we're learning how to separate concerns.
In a 'real' pipeline we'd use something like `curl` (or perhaps `requests` in Python) to load the data into DuckDB.
We'd do it that way to give us more flexibility over things like retries, timeouts, and so on.

The other two tables are ingested in a similar way, except use `CURRENT_TIMESTAMP` for `_latest_reading_at` since the measures and stations APIs don't return any timestamp information.
...
Unlike my link:/2025/03/20/building-a-data-pipeline-with-duckdb/#_joining_the_data[hand-crafted version] for loading the fact table—which required manual steps including pre-creating the table, adding constraints, and so on—dbt comes equipped with a syntax for declaring the _intent_ (just like SQL itself), and at runtime dbt makes it so.
...
What this block does is use a conditional, expressed with `if`/`endif`, wrapped in Jinja code markers `{% %}`, to determine if it's an incremental load.
If it is then the SQL `WHERE` clause gets added.
This is a straightforward predicate, the only difference from vanilla SQL being the `{{ this }}` reference, which compiles into the reference for the table being built, i.e. `fct_readings`.
...
In my link:/2025/03/20/building-a-data-pipeline-with-duckdb/[original version] I use SCD1 and throw away dimension history.
Not for any sound business reason; just because it's the easiest thing to do; drop and recreate the dimension table from the latest version of the source dimension data.

It's kinda a sucky way to do it though because you loose the ability to analyse how dimension data might have changed over time, as well as answer questions based on the state of a dimension at a given point in time.
For example, "What was the total cumulative rainfall in Sheffield in December" could give you a different answer depending on whether you include measuring stations _that were open in December_ or _all those that are open in Sheffield today_.

dbt makes SCD an absolute doddle through the idea of https://docs.getdbt.com/docs/build/snapshots[snapshots].
In a (yet another) good example of how good a fit dbt is for this kind of work, it supports dimension data done 'right' and 'wrong'.
What do I mean by that, and how much heavy lifting are those 'quotation' 'marks' doing?

In an ideal world, where the source data is designed with the data engineer in mind, any time an attribute of a  dimension changes, the data would indicate that with some kind of "last_updated" timestamp.
...
* Raw is literally whatever the API serves us up, plus a timestamp to help us check freshness
...
* Dimension is build from the snapshot table, taking the latest version of each instance of the dimension by checking using `WHERE dbt_valid_to IS NULL`.
...
This is arguably not quite the right configuration—see the note below regarding the `measures` field.
...
image:/images/2026/02/1029TH.png[]
...
[source,sql]
----
🟡◗ select notation,label,dbt_valid_from, dbt_valid_to from snap_stations where notation = 'E6619'
...
🟡◗ select distinct notation,label,measures[1].parameterName
    from snap_stations
    where lcase(label) like '%helebridge%' or lcase(label) like '%mevagissey%'
    order by 2,3;
┌──────────┬───────────────────────────────────────┬─────────────────────────────┐
...
Whilst there might be upstream data issues, sometimes there's self-inflicted mistakes.
Here's one that I realised when I started digging into the data:

[source,sql]
----
🟡◗ SELECT s.notation, s.label, array_length(s.measures) AS measure_count, string_agg(DISTINCT m.parameterName, ', ' ORDER BY m.parameterName) AS parameter_names, s.dbt_valid_from, s.dbt_valid_to
...
Which is indeed what dbt https://docs.getdbt.com/docs/build/snapshots#check-strategy[explicitly recommend that you do]:

> It is better to explicitly enumerate the columns that you want to check.

== The tool that fits like a glove

image:https://media.tenor.com/4o8P4yG5WBsAAAAC/like-a-glove-ace-ventura.gif[]

The above section is a beautiful illustration of _just how much sense the dbt approach makes_.
I spent link:/2025/02/28/exploring-uk-environment-agency-data-in-duckdb-and-rill/[several hours analysing the source data] before trying to build a pipeline.
...
Whilst the data about measuring stations and measurements comes from the API, it's always useful to have a dimension table the provides date information.
...
└────────────┴───────────┴────────────┴────────────────┴─────────────────┴────────────────┴─────────────────┴──────────────┴────────────────┴─────────────────┴──────────────┘
Run Time (s): real 0.006 user 0.001761 sys 0.001160
...
│ 2020-01-01 │   2020    │     1      │ January        │        1        │       3        │ false           │ Wednesday    │       1        │        1        │      1       │
└────────────┴───────────┴────────────┴────────────────┴─────────────────┴────────────────┴─────────────────┴──────────────┴────────────────┴─────────────────┴──────────────┘
Run Time (s): real 0.003 user 0.001164 sys 0.000655
🟡◗
----
...
The source data is duplicated into staging is duplicated into the marts.
...
In addition to this…storage is cheap.
...
dbt gives us a litany of tools which which to encode and enforce it.
...
`SELECT *` might be fast and fun, but it's also dirty af in the long run for a building a pipeline that is stable and self-documenting (of which see below).
...
We looked at https://docs.getdbt.com/reference/resource-properties/freshness[freshness] of source data link:#ingest[above].
...
[source,sql]
----
🟡◗ SELECT notation, freshness_status, last_reading_at, time_since_last_reading, "label" from station_freshness;
┌──────────┬──────────────────┬──────────────────────────┬─────────────────────────┬──────────────────────────────────────────────┐
...
https://docs.getdbt.com/docs/build/unit-tests[Unit tests] are a great way to encode what part of a pipeline is _supposed_ to do.
...
If you were to describe an ideal data pipeline you'd hit the obvious points—clean data, sensible granularity, efficient table design.
...
image:/images/2026/02/dbt-docs.png[]

and its lineage:

image:/images/2026/02/dbt-lineage.png[]
