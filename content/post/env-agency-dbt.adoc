...
TIP: You can find the full dbt project on https://github.com/rmoff/env-agency-dbt/[GitHub here], and link:/2026/02/20/claude-the-instructor/[read about] how I had Claude teach me dbt.
...
With dbt we can configure different levels of freshness check—"_hold up, this thing's getting stale, just letting you know_" (warning), and "_woah, this data source is so old it stinks worse than a student's dorm room, I ain't touching either of those things_" (error).
...
Unlike my link:/2025/03/20/building-a-data-pipeline-with-duckdb/#pass:[_joining_the_data][hand-crafted version] for loading the fact table—which required manual steps including pre-creating the table, adding constraints, and so on—dbt comes equipped with a syntax for declaring the _intent_ (just like SQL itself), and at runtime dbt makes it so.
