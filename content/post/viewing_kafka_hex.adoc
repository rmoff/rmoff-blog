---
title: 'Why JSON isn''t the same as JSONSchema (Viewing Kafka messages bytes as hex)'
aliases:
...
As expected, this works. But what about if we mix it up, and try to read JSON data using the JSONSchema deserialiser (through the `io.confluent.connect.json.JsonSchemaConverter` converter)?
