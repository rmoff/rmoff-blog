...
Linux link:http://docs.oracle.com/cd/E37670_01/E37355/html/ol_about_cgroups.html?ref=rittmanmead.com[Control Groups] (cgroups) are a nifty way to limit the amount of resource, such as CPU, memory, or IO throughput, that a process or group of processes may use. link:https://fritshoogland.wordpress.com/2012/12/15/throttling-io-with-linux/?ref=rittmanmead.com[Frits Hoogland] wrote a great blog demonstrating how to use it to constrain the I/O a particular process could use, and was the inspiration for this one. I have been doing some digging into the performance characteristics of OBIEE in certain conditions, including how it behaves under memory pressure. I'll write more about that in a future blog, but wanted to write this short blog to demonstrate how cgroups can be used to constrain the memory that a given Linux process can be allocated.
...
For more information about the field meaning link:http://docs.oracle.com/cd/E37670_01/E37355/html/ol_memory_cgroups.html?ref=rittmanmead.com[see the doc here].


To test out the cgroup ability to limit memory used by a process we’re going to use the tool *link:http://linux.die.net/man/1/stress?ref=rittmanmead.com[stress]*, which can be used to generate CPU, memory, or IO load on a server. It’s great for testing what happens to a server under resource pressure, and also for testing memory allocation capabilities of a process which is what we’re using it for here.
...
The link:http://man7.org/linux/man-pages/man5/proc.5.html?ref=rittmanmead.com[man page for proc] gives us more information about these fields, but of particular note are:
...
_Thanks to link:https://fritshoogland.wordpress.com/?ref=rittmanmead.com[Frits Hoogland] for reading a draft of this post and providing valuable feedback._
