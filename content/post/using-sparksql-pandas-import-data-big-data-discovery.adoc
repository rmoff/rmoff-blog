...
For this we can use BDD Shell with the Python link:http://pandas.pydata.org/?ref=rittmanmead.com[Pandas] library, and I’m going to do so here through the excellent Jupyter Notebooks interface. You can read more about these link:http://www.rittmanmead.com/2016/06/using-jupyter-notebooks-big-data-discovery-1-2/?ref=rittmanmead.com[here] and details of link:http://www.rittmanmead.com/2016/06/running-big-data-discovery-shell-jupyter-notebook-big-data-lite-vm-4-5/?ref=rittmanmead.com[how to configure them on BigDataLite 4.5 here]. The great thing about notebooks, whether link:http://jupyter.org/?ref=rittmanmead.com[Jupyter] or link:https://zeppelin.apache.org/?ref=rittmanmead.com[Zeppelin], is that I don’t need to write any more blog text here - I can simply embed the notebook inline and it is self-documenting:


link:https://gist.github.com/76b477f69303dd8a9d8ee460a341c445?ref=rittmanmead.com[https://gist.github.com/76b477f69303dd8a9d8ee460a341c445]


(link:https://gist.github.com/76b477f69303dd8a9d8ee460a341c445?ref=rittmanmead.com[gist link])
...
Whilst CSV files are tabular by definition, link:http://www.w3schools.com/json/json_syntax.asp?ref=rittmanmead.com[JSON] records can contain nested objects (recursively), as well as arrays. Let’s look at an example of using link:http://spark.apache.org/docs/latest/sql-programming-guide.html?ref=rittmanmead.com#json-datasets[SparkSQL] to import a simple flat JSON file, before then considering how we handle nested and array formats. Note that SparkSQL can read datasets from both local (`file://`) storage as well as HDFS (`hdfs://`):


link:https://gist.github.com/8b7118c230f34f7d57bd9b0aa4e0c34c?ref=rittmanmead.com[https://gist.github.com/8b7118c230f34f7d57bd9b0aa4e0c34c]


(link:https://gist.github.com/8b7118c230f34f7d57bd9b0aa4e0c34c?ref=rittmanmead.com[gist link])
...
link:https://twitter.com/flederbine/status/752940179569115136?ref=rittmanmead.com[https://twitter.com/flederbine/status/752940179569115136]


Here we use the link:https://cwiki.apache.org/confluence/display/Hive/LanguageManual+LateralView?ref=rittmanmead.com[`LATERAL VIEW`] syntax, with the optional `OUTER` operator since not all tweets have these additional entities, and we want to make sure we show all tweets including those that don’t have these entities. Here’s the SQL formatted for reading:
...
link:https://gist.github.com/a38e853d3a7dcb48a9df99ce1e3505ff?ref=rittmanmead.com[https://gist.github.com/a38e853d3a7dcb48a9df99ce1e3505ff]


(link:https://gist.github.com/a38e853d3a7dcb48a9df99ce1e3505ff?ref=rittmanmead.com[gist link])


You may decide after looking at this that you’d rather just go back to Hive and SerDes, and as is frequently the case in ‘data wrangling’ there’s multiple ways to achieve the same end. The route you take comes down to personal preference and familiarity with the toolsets. In this particular case I'd still go for SparkSQL for the initial exploration as it's quicker to 'poke around' the dataset than with defining and re-defining Hive tables -- link:http://dictionary.cambridge.org/dictionary/english/ymmv?ref=rittmanmead.com[YMMV]. A final point to consider before we dig in is that SparkSQL importing JSON and saving back to HDFS/Hive is a static process, and if your underlying data is changing (e.g. streaming to HDFS from Flume) then you *would* probably want a Hive table over the HDFS file so that it is live when queried.


== Loading an Excel workbook with many sheets


This was the use-case that led me to researching programmatic import of datasets in the first place. I was doing some work with a dataset of link:https://data.gov.uk/dataset/road-accidents-safety-data?ref=rittmanmead.com[road traffic accident data], which included a single XLS file with over 30 sheets, each a lookup table for a separate set of dimension attributes. Importing each sheet one by one through the BDD GUI was tedious, and being a lazy geek, I looked to automate it.
...
link:https://gist.github.com/rmoff/3fa5d857df8ca5895356c22e420f3b22?ref=rittmanmead.com[https://gist.github.com/rmoff/3fa5d857df8ca5895356c22e420f3b22]


(link:https://gist.github.com/rmoff/3fa5d857df8ca5895356c22e420f3b22?ref=rittmanmead.com[gist link])
