...
Here I’m going to demonstrate using JDBC to connect to OBIEE from ODI. It’s a principle that was originally link:https://blogs.oracle.com/dataintegration/entry/connecting_to_oracle_bi_server?ref=rittmanmead.com[written up by Julien Testut here]. We take the OBIEE JDBC driver _bijdbc.jar_ from `$FMW_HOME/Oracle_BI1/bifoundation/jdbc` and copy it to our ODI machine. I’m just using a local agent for my testing, so put it in `~/.odi/oracledi/userlib/`. For a standalone agent it should go in `$AGENT_HOME/odi/agent/lib`.
...
(_For more information on that particular error link:https://www.youtube.com/watch?v=dQw4w9WgXcQ&ref=rittmanmead.com[see here]_)
