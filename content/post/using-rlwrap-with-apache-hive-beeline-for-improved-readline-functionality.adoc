...
rlwrap is a nice little wrapper in which you can invoke commandline utilities and get them to behave with full link:http://www.gnu.org/software/bash/manual/html_node/Readline-Interaction.html?ref=rittmanmead.com#Readline-Interaction[*readline*] functionality just like you’d get at the *bash* prompt. For example, up/down arrow keys to move between commands, but also home/end to go to the start/finish of a line, and even ctrl-R to search through command history to rapidly find a command. It’s one of the standard config changes I’ll make to any system with link:http://www.oracle-base.com/articles/linux/rlwrap.php?ref=rittmanmead.com[Oracle’s *sqlplus*] on, and it works just as nicely with Apache Hive’s commandline interface, *link:https://cwiki.apache.org/confluence/display/Hive/HiveServer2+Clients?ref=rittmanmead.com#HiveServer2Clients-Beeline%E2%80%93NewCommandLineShell[beeline]*.
...
To install rlwrap simply add the link:http://fedoraproject.org/wiki/EPEL?ref=rittmanmead.com#Extra_Packages_for_Enterprise_Linux_.28EPEL.29[EPEL] yum packages to your repository configuration:
...
Now I can connect to beeline, and as before I press up arrow to access commands from when I previously used the tool, but I can also hit link:http://www.gnu.org/software/bash/manual/html_node/Searching.html?ref=rittmanmead.com[ctrl-R to start typing part of a command to recall it], just like I would in bash. Some other useful shortcuts:
...
(link:http://www.bigsmoke.us/readline/shortcuts?ref=rittmanmead.com[more here])
...
rlwrap is the tool that keeps on giving; just as I was writing this article, I noticed that it also auto-highlights opening parentheses when typing the closing one. link:https://www.youtube.com/watch?v=NdWJpHt1Xws&feature=youtu.be&t=24s&ref=rittmanmead.com[Nice]!
