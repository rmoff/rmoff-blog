...
link:http://www.oracle.com/technetwork/middleware/goldengate/overview/index.html[Oracle GoldenGate] for Big Data (OGG BD) supports sending transactions as messages to Kafka topics, both through the link:http://docs.oracle.com/goldengate/bd1221/gg-bd/GADBD/GUID-2561CA12-9BAC-454B-A2E3-2D36C5C60EE5.htm#GADBD449[native Oracle handler] as well as a link:https://blogs.oracle.com/dataintegration/entry/oracle_goldengate_adapter_for_confluent[connector into Confluent's Kafka Connect]. In some research that I was doing with it I found an interesting problem that I am going to demonstrate here and show the troubleshooting tools that may be useful to others encountering similar issues.


The source for the data is link:http://dominicgiles.com/swingbench.html[Swingbench] running against Oracle 12c database (pluggable instance). OGG has been configured as follows:
...
Now let's see if we can find out more. A colleague of mine pointed me towards link:https://docs.oracle.com/goldengate/1212/gg-winux/GLOGD/toc.htm[Logdump], which is link:https://docs.oracle.com/goldengate/1212/gg-winux/GLOGD/wu_logdump.htm[well documented] and also covered by Oracle's A-Team link:http://www.ateam-oracle.com/oracle-goldengate-logdump/[blog here].
...
* Show link:https://docs.oracle.com/goldengate/1212/gg-winux/GLOGD/logdump_commands.htm#GLOGD182[record headers]
...
----
+
* Show link:https://docs.oracle.com/goldengate/1212/gg-winux/GLOGD/logdump_commands.htm#GLOGD159[detailed column data], in both hex and ASCII
...
Interestingly, Google link:https://dbvisit.atlassian.net/wiki/display/REPA11XE/02+-+Initialise+the+Replication[throws up a match for this very column] in which the output of link:http://www.dbvisit.com/products/dbvisit_replicate_real_time_oracle_database_replication/[Dbvisit's replicate tool] run against the Swingbench schema announces:
...
link:https://www.youtube.com/watch?v=uuN7kOW7iwo[Aha]! I spy system generated columns! Let's take a closer look at the `CUST_FUNC_LOWER_NAME_IX` index:
...
Per link:https://docs.oracle.com/goldengate/1212/gg-winux/GIORA/additional_config.htm#GIORA382[Handling Other Database Properties], virtual columns can be handled by using the link:https://docs.oracle.com/goldengate/1212/gg-winux/GWURF/gg_parameters160.htm[`TABLE`] link:https://docs.oracle.com/goldengate/1212/gg-winux/GWURF/gg_parameters160.htm#CEGHBBED[`FETCHCOLS`] configuration on the extract to read the virtual values and `MAP` of the replicat to map them to actual columns on the target. Unfortunately, the system-generated column name isn't accepted by OGG in the `FETCHCOLS` syntax:
