...
Monitoring the health of an OBIEE system and diagnosing problems that may occur is a vital task for the system’s administrator and support staff. It’s one that at Rittman Mead we help customers with implementing themselves, and also provide as a link:http://www.rittmanmead.com/support/[managed service]. In this article I am going to discuss the link:http://www.elasticsearch.org/overview/[ELK stack], which fills a specific gap between the high-level monitoring and configuration functionality of link:https://www.rittmanmead.com/blog/2010/08/obiee-11gr1-basic-system-administration/[Enterprise Manager 11g Fusion Middleware Control], and the Enterprise-grade monitoring, alerting and configuration management of link:https://www.rittmanmead.com/blog/2013/04/obiee-oem12cr2-and-the-bi-management-pack-part-1-introduction-to-oem12cr2/[Enterprise Manager 12c Cloud Control].
...
Out of the box, OBIEE ships with link:http://rittmanmead.com/2011/03/so-how-does-enterprise-manager-work-within-obiee-11g/[Enterprise Manager 11g Fusion Middleware Control] (FMC), which as the name says is part of the Enterprise Manager line of tools from Oracle for managing systems. It is more of a configuration and deployment tool than it is really a monitoring and diagnostics one. The next step up is FMC’s (very) big brother, link:http://www.oracle.com/technetwork/oem/enterprise-manager/overview/index.html[Enterprise Manager 12c Cloud Control] (EM12c). This is very much its own product, requiring its own infrastructure and geared up to monitoring an organisation’s entire fleet of [Oracle] hardware and software. With this greatly enhanced functionally with EM12c also comes a license cost. The ELK stack conceptually fits perfectly alongside your existing EM FMC, providing a most excellent OBIEE monitoring dashboard and analysis tool, and allowing you to explore the kind of diagnostics and historical data that you could have access to in EM 12c.
...
* link:http://www.elasticsearch.org/overview/logstash[Logstash] is an innocuous looking tool that at first glance one could mistakenly write off as “just” a log parser. It does a lot more than that and a healthy ecosystem of _input_, _filter_, _codec_ and _output_ plugins means that it can interface between a great variety of applications, shifting data from one to another and optionally processing and enriching it along the way.
* The final piece of the stack is link:http://www.elasticsearch.org/overview/kibana[Kibana], a web application that enables one to build very flexible and interactive time-based dashboards, sourcing data from ElasticSearch. Interestingly, another of my favourite tools that I have link:https://www.rittmanmead.com/blog/2014/04/extended-visualisation-of-obiee-performance-data-with-grafana/[written about before] - and will write about again in this article - is link:http://grafana.org/[Grafana] which is forked from Kibana (and modified to source its data from time-series databases like link:http://graphite.wikidot.com/[graphite/carbon/whisper] or link:http://influxdb.com/[InfluxDB]) – thus if you’re at home with one you will be the other.


image::/images/2014/10/405celk40.png[]


In this article I’m going to show how to set up your own ELK stack to monitor OBIEE, based on link:http://www.oracle.com/technetwork/middleware/bi-foundation/obiee-samples-167534.html[SampleApp v406].
...
The easiest way to install ElasticSearch is using the link:http://www.elasticsearch.org/guide/en/elasticsearch/reference/current/setup-repositories.html[_yum_ repository]:
...
There’s no repository for logstash, but it’s no biggie because there’s no install as such, just a download and unpack. Grab the download archive for logstash from link:http://www.elasticsearch.org/overview/elkdownloads/[the ELK download page], and then unpack it:
...
Now that we’ve got the software installed, let’s see how it hangs together and create our first end-to-end example. There’s a good link:http://logstash.net/docs/1.4.2/tutorials/getting-started-with-logstash[logstash tutorial here] that covers a lot of the functionality. Here, I’ll just look at some of the very basics, creating a very simple logstash configuration which will prompt for input (i.e. link:http://en.wikipedia.org/wiki/Standard_streams#Standard_input_.28stdin.29[stdin]) and send it straight to ElasticSearch. The _kopf_ plugin that we installed above can show that the data made it to ElasticSeach, and finally we will create a very simple Kibana dashboard to demonstrate its use.
...
So, let us see how to extract the timestamp from the log line. An important part of grok’ing is _patterns_. Grok statement are written as link:http://en.wikipedia.org/wiki/Regular_expression[Regular expressions], or regex (link:http://xkcd.com/208/[obXKCD]), so to avoid continual link:http://en.wikipedia.org/wiki/Reinventing_the_wheel[wheel-reinventing] of regex statements for common objects (time, ip addresses, etc) logstash ships with a bunch of these predefined, and you can build your own too. Taking a line from `nqsserver.log` we can see the timestamp matches the link:http://en.wikipedia.org/wiki/ISO_8601[ISO 8601] standard:
...
+
* link:http://debuggex.com/[debuggex.com]
* link:http://gskinner.com/RegExr/[gskinner.com/RegExr]
* link:http://www.regular-expressions.info/[www.regular-expressions.info]
+

On the Mac there is a useful tool called link:http://www.rwe-uk.com/app/oyster[Oyster] which does the same as the above sites but doesn’t require an Internet connection.
...
Obviously, it would be nice if we could integrate the fundamental time-based nature of both Kibana and Grafana together, so that drilling into a particular time period of interest maybe from an error rate point of view in the logs would also show the system and DMS metrics for the same period. There has been discussion about this (link:http://stackoverflow.com/questions/20040373/kibana-3-milestone-4-and-graphite-integration[1], link:https://github.com/grafana/grafana/issues/17[2], link:https://github.com/grafana/grafana/issues/169[3]) but I don’t get the impression that it will happen soon, if ever. One other item of interest here is link:http://www.elasticsearch.org/blog/building-marvel/[Marvel], which is a commercial offering for monitoring ElasticSearch – through Kibana. It makes use of stock Kibana panel types, along with some new ones including the _Nodes_ panel type, which suits the requirements we have of monitoring OBIEE/system metrics within a Kibana view, but unfortunately it looks like link:https://github.com/elasticsearch/kibana/issues/944[currently it is going to remain within Marvel only].
...
To implement this kind of deployment, you need something like *link:https://github.com/elasticsearch/logstash-forwarder[logstash-forwarder]* on the OBI server which is a light-touch program, sending the messages to logstash itself on the ELK server, over a custom protocol called *link:https://github.com/elasticsearch/logstash-forwarder#future-protocol-discussion[lumberjack]*. Logstash then processes the messages as before, except it is reading the input from the logstash-forwarder rather than from file. An alternative approach to this is using *redis* as a message broker, with logstash running on both the source (sending output to redis) and ELK server (using redis as the input). This approach is documented link:https://kuther.net/blog/indexing-and-searching-weblogic-logs-using-logstash-elasticsearch-and-kibana[very well here] / link:http://michael.bouvy.net/blog/en/2013/11/19/collect-visualize-your-logs-logstash-elasticsearch-redis-kibana/[here], and the former of link:http://michael.bouvy.net/blog/en/2013/12/06/use-lumberjack-logstash-forwarder-to-forward-logs-logstash/[using logstash-forwarder here]. Logstash-forwarder worked very well for me in my tests, and seems to fit the purpose nicely.
