...
This is the second post in a mini-series on monitoring OBIEE. The previous post, link:https://www.rittmanmead.com/blog/2012/09/automated-monitoring-of-obiee-in-the-enterprise-an-overview[Automated Monitoring of OBIEE in the Enterprise – an overview], looked at the overview and theory to why and what we should be monitoring. In this post I am going to walk through implementing a set of automated checks on OBIEE using the Systems Management tool link:http://www.nagios.org/?ref=rittmanmead.com[Nagios]


==== Nagios


There are at least three different flavours of Nagios, and only one of them is free (open source), called *Nagios Core*. The others listed link:http://www.nagios.com/products?ref=rittmanmead.com[here] are Nagios XI and Nagios Fusion.
...
There is documentation provided on how to link:http://assets.nagios.com/downloads/nagioscore/docs/Installing_Nagios_Core_From_Source.pdf?ref=rittmanmead.com[install Nagios from source] which looks comprehensive and easy to follow.


Alternatively, using the link:http://fedoraproject.org/wiki/EPEL?ref=rittmanmead.com[EPEL repository], install nagios and the default set of nagios plugins using the package manager yum: 
...
To successfully work with Nagios it is necessary to understand some of the terminology and object types used. For a complete list with proper definitions, see link:http://nagios.sourceforge.net/docs/3_0/objectdefinitions.html?ref=rittmanmead.com[the documentation].
...
For a detailed explanation of Nagios' plugin architecture, see link:http://nagios.sourceforge.net/docs/3_0/plugins.html?ref=rittmanmead.com[here]
...
link:http://code.google.com/p/logwarn/?ref=rittmanmead.com[check_logwarn] is not provided by the default set of Nagios plugins, and must be downloaded and installed separately. Once installed, it can be used thus:
...
link:http://nagios.sourceforge.net/docs/3_0/toc.html?ref=rittmanmead.com[Nagios Core documentation]
