...
link:https://drill.apache.org/[Apache Drill] enables querying with SQL against a link:https://drill.apache.org/docs/connect-a-data-source/[multitude of datasources] including things like JSON files, Parquet and Avro, Hive tables, RDBMS and more. MapR have released an link:https://drill.apache.org/docs/installing-the-driver-on-linux/#step-1:-download-the-mapr-drill-odbc-driver[ODBC driver] for it, and I thought it'd be neat to get it to work with OBIEE. It link:https://twitter.com/cfiston/status/761269789117517824[evidently does work] for OBIEE running on Windows, but I wanted to be able to use it on my standard environment, Linux.


For more information on Apache Drill, see my previous post, link:http://www.rittmanmead.com/2016/08/an-introduction-to-apache-drill[Introduction to Apache Drill].
...
To get OBIEE to work with Apache Drill we'll use the third option - native ODBC drivers. I'm doing this on link:http://www.oracle.com/technetwork/middleware/bi-foundation/obiee-samples-167534.html[SampleApp] v511. For a geek-out on details of the process I went through and diagnostics used to get this to work, link:https://www.rittmanmead.com/blog/2016/08/how-to-poke-around-obiee-on-linux-with-strace-working-with-unsupported-odbc-sources-in-obiee-12c/[see here].


_Update: link:https://twitter.com/cfiston/status/761269789117517824[Fiston] has written up link:http://obieestuff.blogspot.co.uk/2016/08/using-apache-drill-as-jdbc-data-source.html[an excellent blog post] showing how JDBC can be used to connect OBIEE 12c to Apache Drill - very cool!_
...
Following the documentation, link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1181[Configuring Database Connections Using Native ODBC Drivers]:


. Add the link:https://drill.apache.org/docs/configuring-odbc-on-linux/[necessary environment variables] to BI Server. This is done per-component in a `.properties` file, which for the BI Server (OBIS / nqsserver) is `BI_DOMAIN/config/fmwconfig/bienv/obis/obis.properties`, so on SampleApp `/app/oracle/biee/user_projects/domains/bi/config/fmwconfig/bienv/OBIS/obis.properties`. To this file (which in 12.2.1 is empty by default) we add:
...
One of the many powerful features of Drill is being able to access nested and array JSON values, which I've discussed in my link:http://www.rittmanmead.com/2016/08/an-introduction-to-apache-drill[Introduction to Apache Drill] post. The above examples just use root-level attributes, and could easily be expanded out to process some of the nested fields (such as `hours` in the business data above). For the time being this would be done with DDR, or a Drill view wrapped around it imported into the Physical layer of the RPD.
