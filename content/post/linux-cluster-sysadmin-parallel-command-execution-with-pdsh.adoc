...
PDSH can be installed on the Mac under link:http://brew.sh/?ref=rittmanmead.com[Homebrew] _(did I mention that Rittman Mead laptops are Macs, so I can do all of this straight from my work machine... :-) )_
...
And if you want to run it on Linux from the link:https://fedoraproject.org/wiki/EPEL?ref=rittmanmead.com[EPEL yum repository] (RHEL-compatible, but packages for other distros are available):
...
I started looking into pdsh when it came to setting up a cluster of machines from scratch. One of the must-have tools I like to have on any machine that I work with is the excellent _link:https://sourceforge.net/projects/collectl/?ref=rittmanmead.com[collectl]_. This is an OS resource monitoring tool that I initially learnt of through link:http://kevinclosson.net/?ref=rittmanmead.com[Kevin Closson] and link:http://structureddata.org/?ref=rittmanmead.com[Greg Rahn], and provides the kind of information you’d get from top etc – and then some! It can run interactively, log to disk, run as a service – and it also happens to link:https://www.rittmanmead.com/blog/2014/03/installing-obi-metrics-agent-graphite-and-collectl/[integrate very nicely with graphite], making it a no-brainer choice for any server.
...
*link:http://tldp.org/LDP/abs/html/here-docs.html?ref=rittmanmead.com[Here documents]* (heredocs) are a nice way to embed multi-line content in a single command, enabling the scripting of a file creation rather than the clumsy instruction to “_open an editor and paste the following lines into it and save the file as /foo/bar_”.


Fortunately heredocs work just fine with pdsh, so long as you remember to enclose the whole command in quotation marks. And speaking of which, if you need to include quotation marks in your actual command, you need to escape them with a backslash. Here’s an example of both, setting up the configuration file for my ever-favourite *link:https://twitter.com/rmoff/status/522989362007769088?ref=rittmanmead.com[gnu screen]* on all the nodes of the cluster:
...
You can find out more about PDSH at link:https://code.google.com/p/pdsh/wiki/UsingPDSH?ref=rittmanmead.com[https://code.google.com/p/pdsh/wiki/UsingPDSH]
...
So now your turn – what particular tools or tips do you have for working with a cluster of Linux machines? Leave your answers in the comments below, or tweet them to me at link:https://twitter.com/rmoff/?ref=rittmanmead.com[@rmoff].
