...
The second real advantage of this method is that typically the tools (including the one I have been working with and will demonstrate below, *PhantomCSS*) are based on _the actual engine that drives the web browsers in use by real end-users_. So it’s not a case of parsing the HTML and CSS that the web server sends us and trying to determine if there’s a problem or not - it is actually rendering it the same as Chrome etc and taking a snapshot of it. PhantomCSS uses link:http://phantomjs.org/[PhantomJS], which uses the engine that Safari is built on, WebKit.
...
* link:http://phantomjs.org/[PhantomJS] is the core functionality we’re using, a headless browser sporting Javascript (JS) APIs
* link:http://casperjs.org/[CasperJS] provides a set of APIs on top of PhantomJS that make working with web page forms, navigation etc much easier
* link:https://github.com/Huddle/PhantomCSS[PhantomCSS] provides the regression testing bit, taking snapshots and running code to compare them and report differences.
...
The only bit of the script above that isn’t self-explanatory is where I am referencing elements. The references are as link:http://www.w3.org/TR/selectors/#selectors[CSS3 selectors] and are easily found using something like Chrome Developer Tools. Where the click on Dashboards is simulated, there is a `waitUntilVisible` function, which is crucial for making sure that the page has rendered fully. For a user clicking the menu, they’d obviously wait until it appears but computers work much faster so functions like this are important for reining them back.
...
I have just scratched the surface of what is possible with headless browser scripting for testing OBIEE. Being able to automate and capture the results of browser interactions as we've seen above is hugely powerful. You can find the link:http://docs.casperjs.org/en/latest/modules/casper.html[CasperJS API reference here] if you want to find out more about how it is possible to interact with the web page as a "user".


I’ve put the complete PhantomCSS script link:https://gist.github.com/rmoff/81fcbce6220aeb1580f8[online here]. Let me know in the comments section or link:https://twitter.com/rmoff/[via twitter] if you do try it out!


_Thanks to link:https://twitter.com/Nephentur[Christian Berg] and link:https://twitter.com/G_Ceresa[Gianni Ceresa] for reading drafts of this article and providing valuable feedback._
