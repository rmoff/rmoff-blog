...
_This article was also posted on the link:http://www.confluent.io/blog/streaming-data-oracle-using-oracle-goldengate-kafka-connect/[Confluent blog], head over there for more great Kafka-related content!_


'''


link:http://docs.confluent.io/3.0.0/connect/index.html[Kafka Connect] is part of the link:http://www.confluent.io/product[Confluent Platform], providing a set of connectors and a standard interface with which to ingest data to Kafka, and store or process it the other end. link:http://www.confluent.io/blog/how-to-build-a-scalable-etl-pipeline-with-kafka-connect[Initially launched] with a JDBC source and HDFS sink, the link:http://www.confluent.io/product/connectors[list of connectors] has grown to include a dozen certified connectors, and twice as many again 'community' connectors. These cover technologies such as MongoDB, InfluxDB, Kudu, MySQL - and of course as with any streaming technology, twitter, the de-facto source for any streaming how-to. Two connectors of note that were recently released are for Oracle GoldenGate as a source, and Elasticsearch as a sink. In this article I'm going to walk through how to set these up, and demonstrate how the flexibility and power of the Kafka Connect platform can enable rapid changes and evolutions to the data pipeline.
...
link:https://www.elastic.co/products/elasticsearch[Elasticsearch] is an open-source distributed document store, link:https://www.elastic.co/use-cases[used heavily] for both search, and analytics. It comes with some great tools including link:https://www.elastic.co/products/kibana[Kibana] for data discovery and analysis, as well as a link:https://www.rittmanmead.com/blog/2016/04/experiments-with-elastics-graph-tool/[Graph] tool. Whilst Elasticsearch is capable of being a primary data store in its own right, it is also commonly used as a secondary store in order to take advantage of its rapid search and analytics capabilities. It is the latter use-case that we're interested in here - using Elasticsearch to store a copy of data produced in Oracle.


Confluent's Elasticsearch Connector is an open source connector plug-in for Kafka Connect that sends data from Kafka to Elasticsearch. It is highly efficient, utilising Elasticsearch's link:https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-bulk.html[bulk API]. It also supports all Elasticsearch's data types which it automatically infers, and evolves the Elasticsearch mappings from the schema stored in Kafka records.


Oracle GoldenGate can be used with Kafka to directly stream _every single change made to your database_. Everything that happens in the database gets recorded in the transaction log (OGG or not), and OGG takes that ands sends it to Kafka. In this blog we're using Oracle as the source database, but don't forget that Oracle GoldenGate supports link:http://www.oracle.com/technetwork/middleware/data-integration/goldengate12-2-x-cert-matrix-2769360.xls[many sources]. To use Oracle GoldenGate with Kafka, we use the "link:https://www.oracle.com/goldengate/big-data/index.html[Oracle GoldenGate for Big Data]" version (which has different binaries). Oracle GoldenGate has a significant advantage over the link:http://docs.confluent.io/3.0.0/connect/connect-jdbc/docs/index.html[JDBC Source Connector for Kafka Connect] in that it is a 'push' rather than periodic 'pull' from the source, thus it :
...
Note that Oracle Golden Gate for Big Data also has its own link:http://docs.oracle.com/goldengate/bd1221/gg-bd/GADBD/GUID-2561CA12-9BAC-454B-A2E3-2D36C5C60EE5.htm#GADBD449[native Kafka Handler], which can produce data in various formats directly to Kafka (rather than integrating with the Kafka Connect framework).


== Environment


I'm using the link:http://www.oracle.com/technetwork/database/bigdata-appliance/oracle-bigdatalite-2104726.html[Oracle BigDataLite VM 4.5] as the base machine for this. It includes Oracle 12c, Oracle GoldenGate for Big Data, as well as a CDH installation which provides HDFS and Hive for us to also integrate with later on.
...
To generate the schema and continuous workload, I used link:http://dominicgiles.com/swingbench.html[Swingbench] 2.5.


For a step-by-step guide on how to set up these additional components, see link:https://gist.github.com/rmoff/c3813ec709d66d9a471e2a7f79fc3819[this gist].
...
I'm assuming here that you've already successfully defined and set running an extract against the Swingbench schema (SOE), with a trail file being delivered to `/u01/ogg-bd/dirdat`. For a step-by-step guide on how to do this all from scratch, link:https://gist.github.com/rmoff/5a78bdcd0cf24941ed4b30b604fb44ad[see here].


You can find information about the OGG-Kafka Connect adapter in the link:https://java.net/downloads/oracledi/GoldenGate/Oracle%20GoldenGate%20Adapter%20for%20Kafka%20Connect/README.txt[README here].
...
Edit the existing `/u01/ogg-bd/dirprm/conf.props` and amend `gg.classpath` as shown below. The classpath shown works for BigDataLite - on your own environment you need to make the necessary jar files available per the dependencies listed in the link:https://java.net/downloads/oracledi/GoldenGate/Oracle%20GoldenGate%20Adapter%20for%20Kafka%20Connect/README.txt[README].
...
The message is JSON, so a useful tool to install is link:https://stedolan.github.io/jq/[`jq`]:
...
or even show just sections of the message using jq's syntax (explore it link:https://jqplay.org/[here]):
...
Note that the global `key.ignore` link:https://github.com/confluentinc/kafka-connect-elasticsearch/issues/23[is currently ignored] if you are also overriding another `topic` parameter. If you don't set this flag for the topic, you'll get `org.apache.kafka.connect.errors.DataException: STRUCT is not supported as the document id.`.
...
To configure the dynamic template we'll use the REST API again, and whilst `curl` is fine for simple and repeated command line work, we'll switch to the web-based Elasticsearch REST API client, link:https://www.elastic.co/guide/en/sense/current/introduction.html[Sense]. Assuming that you installed it following the process above you can access it at `http://<server>:5601/app/sense`.


image::/images/2016/10/ogges10.png[]


Click *Get to work* to close the intro banner, and in the main editor paste the following JSON (link:https://gist.github.com/rmoff/4d57a815a539b9f34eeec1ed4cdf8ede[gist here])
...
Kafka and Kafka Connect expose metrics through JMX. There's a variety of tools for capturing, persisting, and visualising this, link:https://softwaremill.com/monitoring-apache-kafka-with-influxdb-grafana/[such as detailed here]. For now, we'll just use JConsole to inspect the metrics and get an idea of what's available.
