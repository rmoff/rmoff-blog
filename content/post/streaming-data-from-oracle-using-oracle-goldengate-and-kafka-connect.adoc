...
. The `dates` template matches any field with `_ts` suffix and sets it to a Date type. The inbound data must match the format shown. For details of the date format specifics, see the link:https://www.joda.org/joda-time/apidocs/org/joda/time/format/DateTimeFormat.html[JodaTime documentation].
