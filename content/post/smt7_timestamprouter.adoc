...
Many sink connectors base the object name to which they stream data on the name of the originating topic. Using the https://docs.confluent.io/platform/current/connect/transforms/timestamprouter.html[`TimestampRouter`] you can modify this topic name in-flight to include the timestamp of the Kafka message. 

To use the Single Message Transform you specify the format of the timestamp to use in the topic name, and the format of the topic name to construct:
