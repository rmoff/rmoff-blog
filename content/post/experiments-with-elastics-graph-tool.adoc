...
Elastic announced their link:https://www.elastic.co/products/graph[Graph] tool at ElastiCON 2016 (link:https://www.elastic.co/elasticon/conf/2016/sf/graph-capabilities-in-the-elastic-stack[see presentation here]). It’s part of the forthcoming link:https://www.elastic.co/v5[X-Pack] which bundles Graph along with other helper tools such as Shield and Marvel. Graph itself is two things; an extension of Elasticsearch’s capabilities, enabling the user to *explore how items indexed in Elasticsearch are related*, and a plugin for Kibana that acts as an optional front-end for this new functionality.


You can find a good introduction to Graph and the purpose and theory behind it in the link:https://www.elastic.co/guide/en/graph/current/graph-introduction.html[documentation here]. The installation of the components themselves is simple and link:https://www.elastic.co/guide/en/graph/current/graph-getting-started.html[documented here].


== First Graph


To use Graph, you just point it at your existing data in Elasticsearch. The first data set I’m going to explore is one of the standard ones that everyone uses; Twitter. I'm streaming it in through Logstash (via link:http://rmoff.net/2016/04/12/decoupling-the-data-pipeline-with-kafka-a-very-simple-real-life-example/[Kafka for flexibility]), but if you wanted you could link:https://www.elastic.co/blog/visualising-oracle-performance-data-with-the-elastic-stack[ship it in via JDBC] from any RDBMS, or from link:https://www.rittmanmead.com/blog/2014/11/analytics-with-kibana-and-elasticsearch-through-hadoop-part-2-getting-data-into-elasticsearch/[HDFS] too. +
...
Let’s start with a blank canvas, in basic mode, showing hashtags related to … me (link:https://twitter.com/rmoff[@rmoff])!
...
link:https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations-bucket-significantterms-aggregation.html[Elasticsearch's documentation] describes the significant terms algorithm thus, using the example of suggesting "H5N1" when users search for "bird flu" in text:
...
More information about the scoring can be link:https://www.elastic.co/guide/en/elasticsearch/guide/current/scoring-theory.html[found here], which includes the fact that the scoring is, in part, based on link:https://en.wikipedia.org/wiki/Tf%E2%80%93idf[TF-IDF] (Term Frequency-Inverse Document Frequency).


== Licensing


Graph requires a licence - link:https://www.elastic.co/subscriptions[see here for details].
...
For a well-informed view on Graph theory and Social Network Analysis (SNA), see Jordan Meyer's link:https://s3.amazonaws.com/rmc_docs/RMOUG2013%20-%20SNA%20with%20Oracle.pdf[presentation here] (and link:https://github.com/jordanmeyer/oracle-sna-hol[associated R code]), as well as Mark Rittman's link:http://www.slideshare.net/rittmanmead/oracle-big-data-spatial-graph-social-media-analysis-case-study[presentation from BIWA this year.]


== Footnote: The Twitter Dataset


The dataset I’m using is a live stream from Twitter, via Logstash link:http://rmoff.net/2016/04/12/decoupling-the-data-pipeline-with-kafka-a-very-simple-real-life-example/[and Kafka], searching for a set of terms related to me and the field I work in. Therefore, there’s going to be a bunch of relationships missing (if I’ve not included the relevant term in my tweet search), and relationships over-stated (because as a proportion of all the records the terms I’ve selected will dominate). +
