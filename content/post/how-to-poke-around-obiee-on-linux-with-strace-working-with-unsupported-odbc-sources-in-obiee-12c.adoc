...
OBIEE 12c (and 11g and 10g before it) supports link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1465[three primary ways] of connecting to data sources:


. link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1179[Native Gateway], such as OCI for Oracle. This is always the preferred option as it gives the greatest support and performance.
. link:https://www.progress.com/datadirect-connectors[Data Direct ODBC Drivers], a set of which are bundled with OBIEE for enabling connectivity to sources such as SQL Server, MySQL, Hive, and Impala. The configuration of these is link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1194[documented in the OBIEE manuals] and is generally a supported configuration.
. link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1181[Native ODBC Drivers].


_There's also kind of a fourth option, which is link:http://obieestuff.blogspot.co.uk/2016/08/using-apache-drill-as-jdbc-data-source.html[JDBC], as explained by link:https://twitter.com/cfiston/[Fiston] in link:http://obieestuff.blogspot.co.uk/2016/08/using-apache-drill-as-jdbc-data-source.html[this excellent post]. It's OBIEE 12c only and not fully documented/supported._
...
ODBC (link:https://en.wikipedia.org/wiki/Open_Database_Connectivity[Open Database Connectivity]) is a documented API designed to enable applications to work with databases, without one explicitly supporting the other. A comparable protocol is link:https://en.wikipedia.org/wiki/Java_Database_Connectivity[JDBC], which is also widely used (but as yet not fully supported/documented within OBIEE). To use native ODBC drivers with OBIEE on *nix, you install the driver on your OBIEE server and then configure OBIEE to use it. Mark Rittman wrote an example of how to do this link:http://www.rittmanmead.com/2013/04/obiee-odi-and-hadoop-part-2-connecting-obiee-11-1-1-7-to-hadoop-data-sources/[with the original Hive ODBC drivers here] (before they were formally bundled with OBIEE), and link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1181[the manual] shows how to use it with a native driver for Teradata.


My interest in this is the link:https://drill.apache.org/[Apache Drill] tool, which enables querying with SQL against a link:https://drill.apache.org/docs/connect-a-data-source/[multitude of datasources], including things like JSON files, Hive tables, RDBMS, and more. MapR have released an link:https://drill.apache.org/docs/installing-the-driver-on-linux/#step-1:-download-the-mapr-drill-odbc-driver[ODBC driver] for it, and I thought it'd be neat to get link:https://www.rittmanmead.com/blog/2016/08/using-apache-drill-with-obiee-12c/[Apache Drill to work with OBIEE]. It link:https://twitter.com/cfiston/status/761269789117517824[evidently does work] for OBIEE running on Windows, but I wanted to be able to use it on my standard environment, Linux. This blog post is a warts-and-all exploration of the process I went through to get it to work, since I thought it might be of interest to see some of the forensic methods available when trying to get things to work. For just the headlines, see link:https://www.rittmanmead.com/blog/2016/08/using-apache-drill-with-obiee-12c/[Using Apache Drill with OBIEE 12c].
...
For more examples of Drill, see the link:https://drill.apache.org/docs/analyzing-the-yelp-academic-dataset/[tutorials].
...
Per the link:https://drill.apache.org/docs/installing-the-driver-on-linux/#system-requirements[system requirements], we need to make sure that we're using one of the supported ODBC Driver Managers, so we'll install iODBC to start with (the other option being unixODBC):
...
Now follow the link:https://drill.apache.org/docs/configuring-odbc-on-linux/[configuration instructions]. To start with we'll do this in isolation of OBIEE to check that it works, and then bring it into OBIEE's world (for example, `odbc.ini` already exists in OBIEE).
...
We've shown that we can connect to Apache Drill using ODBC and query it. Let's see if we get it to work for OBIEE. Our starting point is the 12c docs, link:https://docs.oracle.com/middleware/1221/biee/BIEMG/deploy_rpd.htm#BIEMG1181[Configuring Database Connections Using Native ODBC Drivers].
...
We can check that the BI Server has picked up our new environment variables by using the link:http://man7.org/linux/man-pages/man5/proc.5.html[`/proc` pseudo file-system] (you can also see an environment variable dump as part of the `obis1.out` logfile during startup):
...
Signal 11 is "SIGSEGV", or segmentation fault, which is bad, link:https://www.youtube.com/watch?v=KqOsrniBooQ[m'kay]?
...
One of the routes of investigation is the Driver Manager. This is for two reasons; one is that on Windows ODBC is native, the driver manager is built in. As mentioned, link:https://twitter.com/cfiston/[@cfiston] has link:https://twitter.com/cfiston/status/761269789117517824[got this to work] - but on Windows. So platform differences aside (he says casually), this could be something. The second reason is the difference in the driver manager that shows in the ODBC log between a successful (via `iodbctest`) request
...
From link:https://drill.apache.org/docs/configuring-odbc-on-linux/#configuring-.mapr.drillodbc.ini:[https://drill.apache.org/docs/configuring-odbc-on-linux/#configuring-.mapr.drillodbc.ini:]
...
And if you do want the tl;dr of getting OBIEE to work with Apache Drill, link:https://www.rittmanmead.com/blog/2016/08/using-apache-drill-with-obiee-12c/[it's written up using the ODBC driver over here], and with the JDBC driver in link:https://twitter.com/cfiston/[Fiston]'s link:http://obieestuff.blogspot.co.uk/2016/08/using-apache-drill-as-jdbc-data-source.html[blog post here].
