...
Let's now explore the different alternatives to MinIO, and how easy they are to switch out MinIO for.
...
NOTE: One thing I did notice was this one of the projects that S3Proxy uses, https://jclouds.apache.org/[jclouds], was moved to the Apache Attic (i.e. retired) in mid-2025—although probably nbd if you're only using local storage anyway?


=== https://github.com/rustfs/rustfs[RustFS]

💾 https://github.com/rmoff/minio-alternatives/blob/main/rustfs/docker-compose.yml[Complete Complete test code]
...
💾 https://github.com/rmoff/minio-alternatives/blob/main/cloudeserver/docker-compose.yml[Example Docker Compose]
...
💾 https://github.com/rmoff/minio-alternatives/blob/main/apache-ozone/docker-compose.yml[Complete Complete test code]
...
|===
| Name | Ease of config | Licence | Commercial Backing & Governance | Docker pulls^1^ | GH Stars | Date of first commit | Main Committers
...
I mean no cast no shade on those options against which I've not recorded a `yes`; they're probably excellent projects, but just not focussed on my primary use case (simple & easy to configure single-node local S3).
