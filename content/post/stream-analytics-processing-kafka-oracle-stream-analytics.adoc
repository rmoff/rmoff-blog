...
Data in Kafka can be serialised in many formats, including link:https://avro.apache.org/?ref=rittmanmead.com[Avro] - which OSA doesn’t seem to like. No error is thrown to the GUI but the exploration remains blank.
...
One of the challenges that I found working with OSA was defining the “Shape” (data model) of the inbound stream data. JSON is a format used widely as a technology-agnostic data interchange format, including for the twitter data that I was working with. You can see a link:http://www.jsoneditoronline.org/?id=47c13a332037ddebfa046fdfc29cce7f&ref=rittmanmead.com[sample record here]. One of the powerful features of JSON is its ability to nest objects in a record, as well as create arrays of them. You can read more about this detail in a recent article link:http://www.rittmanmead.com/2016/07/using-sparksql-pandas-import-data-big-data-discovery/?ref=rittmanmead.com[I wrote here]. Unfortunately it seems that OSA does not support flattening out JSON, meaning that only elements in the root of the model are accessible. For twitter, that means we can see the text, and who it was in reply to, but not the user who tweeted it, since the latter is a nested element (along with many other fields, including hashtags which are also an array):
...
So what to do if the inbound streaming data is in nested-JSON format? It seems to me the only option is to pre-process it to flatten it. There are a variety of tools that could be used here - in the first instance I’d generally reach for link:https://www.elastic.co/products/logstash?ref=rittmanmead.com[Logstash], it being the one I’m most familiar with. To get an idea of the schema of a JSON record you can use link:http://jsonschema.net/?ref=rittmanmead.com#/[jsonschema.net]. Funnily enough when I was researching this blog post I came across the exact same link:https://discuss.elastic.co/t/flatten-json-array-in-logstash/399/4?ref=rittmanmead.com[problem on a forum posted by … me]! Early last year I was working with the same dataset, and had the same issue with embedded arrays. The way to do it in Logstash is with a bit of Ruby code to flatten the arrays, and a standard `mutate` to bring nested objects up to the root level. Sample code:
...
You can find the full link:https://gist.github.com/rmoff/6cb9e28245cde010e6f694348286046c?ref=rittmanmead.com[Logstash code on gist here]. With this logstash code running I set up a new OSA Stream pointing to the *new* Kafka topic that Logstash was writing, and added the flattened fields to the Shape:
...
I can see OSA being used in two ways. The first as an ‘endpoint’ for streams with users taking actions based on the data, with some of the link:https://docs.oracle.com/middleware/12211/osa/getting-started-streamanalytics/GUID-4C3BD7E9-A1BB-4079-9C34-3D239E9B2CE2.htm?ref=rittmanmead.com#SXGSG262[use cases] listed here. The second is for prototyping transformations and analyses on streams prior to productionising them. The visual interface and immediacy of feedback on transformations applied means that users can quickly understand what further processing they may want to apply to the stream using actual streaming data to inform this.


This latter concept - that of prototyping - is similar to that which we see with another of Oracle’s products, Big Data Discovery. With BDD users can analyse data in the organisation’s data reservoir, as well as apply transformations to it (link:https://speakerdeck.com/rmoff/unlock-the-value-in-your-big-data-reservoir-using-oracle-big-data-discovery-and-oracle-big-data-spatial-and-graph?ref=rittmanmead.com[read more]). Just as BDD doesn’t replace OBIEE or Visual Analyzer but enables users to understand how they do want to model the data in these tools, OSA wouldn’t replace “production grade” integration done by Oracle Data Integrator. What it would do is allow users to get a clearer idea of the transformations they would want performed in it.


OSA's user interface is easy to use and intuitive, and this is definitely a tool that you would put in front of technically minded business users. There are limitations to what can be achieved technically through the web GUI alone and something like Oracle Data Integrator (ODI) would still be a more appropriate fit for complex streaming work. At Oracle Open World last year link:http://www.rittmanmead.com/2015/11/oracle-openworld-2015-roundup-part-2-data-integration-and-big-data-in-the-cloud/?ref=rittmanmead.com[it was announced] (link:http://s3.amazonaws.com/ppt-download/con9737-dis-overviewroadmapslideshare-151023012104-lva1-app6891.pdf?response-content-disposition=attachment&Signature=40y%2B6Oo41WQ8ViK1dxQpJDbCJPQ%3D&Expires=1469188005&AWSAccessKeyId=AKIAJ6D6SEMXSASXHDAQ&ref=rittmanmead.com[slides]) that a beta would be starting for ODI using Spark Streaming for ETL and stream processing, so it'll be interesting to see this when it comes out.


=== Further Reading


* link:https://guidoschmutz.wordpress.com/2016/06/12/providing-oracle-stream-analytics-12c-environment-using-docker/?ref=rittmanmead.com[Providing Oracle Stream Analytics 12c environment using Docker], by Guido Schmutz
* link:https://www.linkedin.com/pulse/real-time-data-streaming-exploration-oracle-stream-analytics-hijazi-1?ref=rittmanmead.com[Real-Time Data Streaming & Exploration with Oracle GoldenGate & Oracle Stream Analytics], by Issam Hijazi
* link:https://technology.amis.nl/2015/04/12/demonstration-of-oracle-stream-explorer-for-live-device-monitoring-collect-filter-aggregate-pattern-match-enrich-and-publish/?ref=rittmanmead.com[Demonstration of Oracle Stream Explorer for live device monitoring – collect, filter, aggregate, pattern match, enrich and publish] by Lucas Jellema
* link:http://www.oracle.com/technetwork/middleware/complex-event-processing?ref=rittmanmead.com[OTN OSA home page]
* link:https://docs.oracle.com/middleware/12211/osa/index.html?ref=rittmanmead.com[Documentation index]

+
* link:https://docs.oracle.com/middleware/12211/osa/getting-started-streamanalytics/GUID-4C3BD7E9-A1BB-4079-9C34-3D239E9B2CE2.htm?ref=rittmanmead.com#SXGSG262[Use cases]
* link:https://docs.oracle.com/middleware/12211/lcm/INOEP/toc.htm?ref=rittmanmead.com#INOEP-GUID-8CE0F958-876D-433C-939B-83A8C03A5FBE[Installation]
+
