...
original_url: "https://www.decodable.co/blog/adventures-with-apache-flink-and-delta-lake"
---
...
link:https://delta.io/[Delta Lake]  (or Delta, as it’s often shortened to) is an open-source project from the Linux Foundation that’s primarily backed by Databricks.
It’s an open table format (OTF) similar in concept to Apache Iceberg and Apache Hudi.
Having  link:https://www.decodable.co/blog-author/robin-moffatt[previously]  dug into using Iceberg with both  link:/2024/07/18/sending-data-to-apache-iceberg-from-apache-kafka-with-apache-flink/[Apache Flink]  and  link:/2024/06/18/how-to-get-data-from-apache-kafka-to-apache-iceberg-on-s3-with-decodable/[Decodable] , I wanted to see what it was like to use Delta with Flink—and specifically, Flink SQL.
...
Just a tip before we begin: regardless of table format, if you’re using Flink you need to understand catalogs and catalog metastores before going any further.
The good news for you is that I’ve written both a  link:/2024/02/16/catalogs-in-flink-sqla-primer/[primer]  and detailed  link:/2024/02/19/catalogs-in-flink-sqlhands-on/[hands-on guide] .
Once you’ve understood catalogs, you’ll also want to have an appreciation of  link:/2024/02/27/flink-sql-and-the-joy-of-jars/[JAR management in Flink] .

This blog post follows on from the one that prompted it,  link:/2024/08/06/troubleshooting-flink-sql-s3-problems/[Understanding Flink and S3 configuration] , since as you’ll see later this is the very nub of the challenge that I had getting Delta to work with Flink SQL.
To begin with, I’ll give you the conclusion, and then work backwards from there.
...
----
_Note that I’m using MinIO, which is S3-compatible._
_If you’re using real S3 then you oughtn’t and need the_`endpoint`_and_`path.style.access`_configuration._
...
----
3.
Add the following JARs to Flink’s `./lib` folder:
...
Add the followings JARs to Flink’s `./lib` folder:
...
I’ve built out a self-contained stack so that you can experiment with running Flink SQL and Delta Lake together.
It’s on the  link:https://github.com/decodableco/examples/[Decodable examples repository] .
...
----
This should result in a `t_foo` table written on S3 (MinIO), which you can verify with MinIO’s `mc` command:
...
_Unfortunately DuckDB Delta_ link:https://github.com/duckdb/duckdb_delta/issues/14[doesn’t seem to yet support MinIO] _, but if I re-run the test against a real S3 bucket we can query the Delta Lake data successfully_:
...
Above is what works, since that’s usually what people find most useful.
...
Since I’m using MinIO (an S3-compatible object store) the additional `endpoint` and `path.style.access` configuration is needed.
...
----
I also set detailed logging for several components including Delta by adding this to Flink’s `log4j.properties` and `log4j-cli.properties`:
...
----
This class is part of `delta-storage`, so let’s add `delta-storage-3.2.0.jar` to the `./lib` folder and try again.

(At this point I’m not going to repeat '_and then I bounced the stack_'; take it as a given that after each thing I changed, I bounced the stack and reran the test, using the bash code I showed above.)
...
Caused by: java.lang.RuntimeException: java.lang.ClassNotFoundException: Class org.apache.hadoop.fs.s3a.S3AFileSystem not found
    ...
    ... 16 more
----
What’s really puzzling me here is that `S3AFileSystem` was found just fine on exactly the same deployment when I used it to write my  link:/2024/08/06/troubleshooting-flink-sql-s3-problems/[previous article]  about S3 troubleshooting.
All that’s changed is setting the Hadoop classpath, and adding the Delta JARs:
...
----
But for whatever reason isn’t being used.
Let’s add `hadoop-aws` because the docs say so, and see if it helps:
...
Credentials, end points, it’s all good.
But switch to writing to S3 via the Delta connector and everything goes screwy.

Heading to the `sql-client` log file we can see that our existing `flink-conf.yaml` configuration for S3 just isn’t being used, as evidenced by the lack of a custom S3 endpoint being recognised (see my  link:/2024/08/06/troubleshooting-flink-sql-s3-problems/[previous post]  for more detail):
...
----
And thus, the `SimpleAWSCredentialsProvider` reports `No AWS credentials in the Hadoop configuration…`
...
----
…leaving authentication to failback to the `EnvironmentVariableCredentialsProvider` (which also doesn’t work because I’ve not set any credentials in the environment variables):
...
> This class was backported from Flink’s flink-hadoop-fs module, and it contains a subset of methods comparing to the original class.
...
----
I talked about `FLINK_CONFIG_PREFIXES` in my previous  link:/2024/08/06/troubleshooting-flink-sql-s3-problems/[Flink S3 troubleshooting post] .
Taking a bit of a punt, let’s change our `flink-conf.yaml` and add this `flink.hadoop.` prefix:
...
----
So similar to the previous (but different 😡) S3 configuration handling, what’s needed in the Flink configuration file isn’t what the `S3AFileSystem` module is using (or logging), which is all `fs.s3a.` prefixed.
...
----
Google wasn’t much help on this one, but I eventually found  link:https://github.com/delta-io/delta/blob/70bfe82e40db6ba1fd0fe591d635bef5e177411f/build.sbt#L1020[this line]  in the Delta build script which references `com.chuusai` which pointed me to  link:https://repo1.maven.org/maven2/com/chuusai/shapeless_2.12/2.3.4/shapeless_2.12-2.3.4.jar[shapeless_2.12-2.3.4.jar]  on Maven._
...
Nope.
At least, not yet.
Let’s chuck the `flink-sql-parquet` JAR into `./lib` and see if that helps.
...
It’s there, in black and white (or whatever l33t colour schema you have your terminal set to):
...
. `s3a://warehouse/_delta_log/_last_checkpoint` (which doesn’t exist, since we’ve only just created the table)
. `s3a://warehouse/_delta_log` (which doesn’t exist; it’s a 'folder', not an object)
...
Searching in the log for the file name shows that the upload appears before the S3 interactions in the log that I’ve noted above:
...
DEBUG org.apache.hadoop.fs.s3a.AWSCredentialProviderList           [] - Using credentials from SimpleAWSCredentialsProvider
----
Something is playing silly buggers here 🤔.
...
Let’s add the original format of S3 config back into `flink-conf.yaml`, so it’s duplicated:
...
Since we’re into the realms of trying random shit, let’s think about the `S3AFileSystem` dependency.
If we’re just writing vanilla JSON (for example) from Flink, we add the `flink-s3-fs-hadoop` JAR.
But as we saw above, that wasn’t enough for Delta Lake, and we had to add `hadoop-aws`.
...
On that basis, let’s get rid of `flink-s3-fs-hadoop` temporarily, and test this hypothesis.
...
----
There we go—it’s a double act show, with `hadoop-aws` *and* `flink-s3-fs-hadoop` being responsible for writing to S3, but using _different_ configuration parameters.
...
----
Which then turned up the following—note the different `tid` values:
