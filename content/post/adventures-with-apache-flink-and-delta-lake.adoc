...
The good news for you is that I’ve written both a  link:https://www.decodable.co/blog/catalogs-in-flink-sql-a-primer[primer]  and detailed  link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[hands-on guide] .
Once you’ve understood catalogs, you’ll also want to have an appreciation of  link:https://www.decodable.co/blog/flink-sql-and-the-joy-of-jars[JAR management in Flink] .

This blog post follows on from the one that prompted it,  link:https://www.decodable.co/blog/troubleshooting-flink-sql-s3-problems[Understanding Flink and S3 configuration] , since as you’ll see later this is the very nub of the challenge that I had getting Delta to work with Flink SQL.
...
Caused by: java.lang.RuntimeException: java.lang.ClassNotFoundException: Class org.apache.hadoop.fs.s3a.S3AFileSystem not found
    ...
    ... 16 more
----
What’s really puzzling me here is that `S3AFileSystem` was found just fine on exactly the same deployment when I used it to write my  link:https://www.decodable.co/blog/troubleshooting-flink-sql-s3-problems[previous article]  about S3 troubleshooting.
...
Heading to the `sql-client` log file we can see that our existing `flink-conf.yaml` configuration for S3 just isn’t being used, as evidenced by the lack of a custom S3 endpoint being recognised (see my  link:https://www.decodable.co/blog/troubleshooting-flink-sql-s3-problems[previous post]  for more detail):
