...
I’ve written  link:https://www.decodable.co/blog/kafka-to-iceberg-with-flink[before]  about using Flink SQL for ETL pipelines.
At a very high level, the pattern looks like this:

. Get your head around  link:https://www.decodable.co/blog/catalogs-in-flink-sql-a-primer[Flink catalogs]  and then  link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[try them out]  until you’ve decided which you’ll use. That, or just use the in-memory Flink catalog and get used to having to re-run all your DDL each time you restart your Flink SQL session…
