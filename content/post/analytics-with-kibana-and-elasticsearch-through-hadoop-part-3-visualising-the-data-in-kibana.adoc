...
I’ve been looking at both the current release version of Kibana (3.x), and also the link:http://www.elasticsearch.org/blog/kibana-4-beta-1-released/[beta of Kibana 4] which brings with it a very smart visualiser that we’ll look at in detail. It looks like Kibana 4 is a ground-up rewrite rather than modifications to Kibana 3, which means that at the moment it is a long way from parity of functionality - which is why I’m flitting between the two. For a primer in Kibana 3 and its interface see my article on using it to monitor OBIEE.


Installing Kibana is pretty easy in Kibana 3, involving a simple config change to a web server of your choice that you need to provide (details in my previous blog), and has been made even easier in Kibana 4 which actually ships with its own web server so you literally just link:https://github.com/elasticsearch/kibana[download it], unarchive it and run it.
...
NB at the moment in Kibana 4 link:https://github.com/elasticsearch/kibana/issues/1784[intervals are fixed] (in Kibana 3 they were freeform).
...
It’s in the current lack of a table visualisation that Kibana 4 is currently limited (link:https://github.com/elasticsearch/kibana/pull/1763[although it is planned]), because this analysis here (of the top three authors, what were their respective two most popular posts) just makes no sense as a graph:
...
Whilst you can access the table on a dashboard, it doesn’t persist as the default option of the view, always showing the graph initially. As noted above, a table visualisation is link:https://github.com/elasticsearch/kibana/pull/1763[planned and under development] for Kibana 4.
