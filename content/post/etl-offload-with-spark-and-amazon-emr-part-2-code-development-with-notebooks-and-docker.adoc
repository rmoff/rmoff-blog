...
Whilst others in Rittman Mead have done lots of work with Spark, I myself was new to it, and needed a sandpit in which I could flail around without causing any real trouble. Thus I set up a nice self-contained development environment on my local machine, using link:https://www.docker.com/?ref=rittmanmead.com[Docker] to provision and host it, and link:http://jupyter.org/?ref=rittmanmead.com[Jupyter Notebooks] as the interface.
...
Notebooks are portable, meaning that you can work with them in one system, and then upload them for others to learn from and even import to run on their own systems. I've shared a simplified version of the notebook that I developed for this project link:https://gist.github.com/rmoff/37990bc30c0e3c155d611ad20812877a?ref=rittmanmead.com[on gist here], and you can see an embedded version of it at the end of this article.


The two most common are link:https://zeppelin.apache.org/?ref=rittmanmead.com[Apache Zeppelin], and link:http://jupyter.org/?ref=rittmanmead.com[Jupyter Notebooks] (previously known as iPython Notebooks). Jupyter is the one I've used previously, and stuck with again here. To read more about notebooks and see them in action, see my previous blog posts link:https://www.rittmanmead.com/blog/2016/07/using-r-jupyter-notebooks-big-data-discovery/[here] and link:https://www.rittmanmead.com/blog/2016/07/using-sparksql-pandas-import-data-big-data-discovery/[here].


=== Docker


Plenty's been written about link:https://www.docker.com/?ref=rittmanmead.com[Docker]. In a nutshell, it is a way to provision and host a set of self-contained software. It takes the idea of a virtual machine (such as VMWare, or VirtualBox), but without having to install an OS, and then the software, and then configure it all yourself. You simply take a "Dockerfile" that someone has prepared, and run it. You can create copies, or throwaway and start again, from a single command. I ran Docker on my Mac through link:https://kitematic.com/?ref=rittmanmead.com[Kitematic], and natively on link:http://rmoff.net/2016/06/07/a-new-arrival/?ref=rittmanmead.com[my home server].


There are prebuilt Docker configuration files for lots of software (including link:https://github.com/oracle/docker-/blog/content/images/2016/12/tree/master/OracleDatabase?ref=rittmanmead.com[Oracle] and link:https://gianniceresa.com/2016/09/a-full-obiee-in-1-command-docker-compose-can-do-it/?ref=rittmanmead.com[OBIEE]!), and I link:https://github.com/jupyter/docker-stacks/tree/master/all-spark-notebook?ref=rittmanmead.com[found one that includes Spark, PySpark, and Jupyter] - perfect!
...
With my development environment up and running, I set to writing the Spark job. Because I'm already familiar with Python I took advantage of link:http://spark.apache.org/docs/latest/api/python/?ref=rittmanmead.com[PySpark]. Below I describe the steps in the processing and how I achieved them.
...
We use the link:http://docs.pythonboto.org/en/latest/?ref=rittmanmead.com[`boto`] library to interact with S3 to find a list of files ('keys') matching the pattern of source files that we want to process.
...
The Docker image I was using was running Spark 1.6, so I was using the link:https://github.com/databricks/spark-csv?ref=rittmanmead.com[Databricks CSV reader]; in Spark 2 this is now link:http://spark.apache.org/docs/latest/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.DataFrameReader.csv[available natively]. The CSV file is loaded into a Spark data frame. Note that Spark is reading the CSV file directly from a S3 path.
...
The filename of the source data includes a country field as part of it. Here we use link:http://rubular.com/r/S1h8aAHM7l?ref=rittmanmead.com[this regular expression] to extract it:
...
Note that the link:https://spark.apache.org/docs/1.6.2/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.DataFrame.withColumn[`withColumn`] function requires a Column value, which we create here using the PySpark link:https://spark.apache.org/docs/1.6.2/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.functions.lit[`lit`] function that was imported earlier on.
...
Then some light data cleansing with the link:http://spark.apache.org/docs/latest/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.DataFrame.filter[`filter`] function to remove blank `SITE` entries, and blank `SITE_RETAIL_TYPE` entries:
...
Using the link:http://spark.apache.org/docs/latest/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.DataFrame.filter[`filter`] function and link:http://spark.apache.org/docs/latest/api/python/pyspark.sql.html?ref=rittmanmead.com#pyspark.sql.DataFrame.show[`show`] we can inspect the dataset for matches, and misses:
...
(link:https://en.wiktionary.org/wiki/for_the_win?ref=rittmanmead.com[FTW])
