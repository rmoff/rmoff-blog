...
We recently undertook a two-week Proof of Concept exercise for a client, evaluating whether their existing ETL processing could be done faster and more cheaply using Spark. They were also interested in whether something like link:http://docs.aws.amazon.com/redshift/latest/mgmt/welcome.html?ref=rittmanmead.com[Redshift] would provide a suitable data warehouse platform for them. In this series of blog articles I will look at how we did this, and what we found.
...
Since the client already uses Amazon Web Services (AWS) for all its infrastructure, it made sense to remain in the AWS realm for the first round of investigation. We broke the overall requirement down into pieces, so as to understand (a) the most appropriate tool at each point and (b) the toolset with best overall fit. A very useful reference for an Amazon-based big data design is the presentation link:http://www.slideshare.net/AmazonWebServices/big-data-architectural-patterns-and-best-practices-on-aws?ref=rittmanmead.com[Big Data Architectural Patterns and Best Practices on AWS]. Even if you're not running on AWS, the presentation has some useful pointers for things like where to be storing your data based on volumes, frequency of access, etc.


=== Data Ingest


The starting point for the data was link:http://docs.aws.amazon.com/AmazonS3/latest/dev/Welcome.html?ref=rittmanmead.com[Amazon's storage service - S3], in which the data files in CSV format are landed by an external process every hour.
...
+
* Use Hadoop-based processing, running on link:https://aws.amazon.com/emr/?ref=rittmanmead.com[Elastic Map Reduce (EMR)]:
...
We opted to use Spark to process the files, joining them to the reference data, and carrying out de-duplication. For a great background and discussion on Spark and its current place in data architectures, have a link:http://www.drilltodetail.com/podcast/2016/11/1/drill-to-detail-ep7-apache-spark-and-hadoop-application-architectures-with-special-guest-mark-grover?ref=rittmanmead.com[listen to this podcast].
...
+
* _With the data at rest on S3, Amazon's link:https://aws.amazon.com/athena/?ref=rittmanmead.com[Athena] is also of interest here, but was released after we carried out this particular investigation._
+


The presumption was that OBIEE would continue to provide the front-end to the analytics. Oracle's link:http://www.oracle.com/technetwork/middleware/oracle-data-visualization/downloads/oracle-data-visualization-desktop-2938957.html?ref=rittmanmead.com[Data Visualization Desktop] tool was also of interest.
