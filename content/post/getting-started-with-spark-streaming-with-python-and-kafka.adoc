...
In this article I am going to look at link:https://spark.apache.org/streaming/[Spark Streaming]. This is one of several libraries that the link:https://spark.apache.org/[Spark platform] provides (others include link:https://spark.apache.org/sql/[Spark SQL], link:https://spark.apache.org/mllib/[Spark MLlib], and link:https://spark.apache.org/graphx/[Spark GraphX]). Spark Streaming provides a way of processing "unbounded" data - commonly referred to as "streaming" data. It does this by breaking it up into microbatches, and supporting windowing capabilities for processing across multiple batches. You can read more in the excellent link:https://spark.apache.org/docs/latest/streaming-programming-guide.html[Streaming Programming Guide].


image::/images/2017/01/streaming-flow.png[]


(link:https://spark.apache.org/docs/latest/streaming-programming-guide.html[image src])
...
* link:https://www.oreilly.com/ideas/the-world-beyond-batch-streaming-101[The world beyond batch: Streaming 101]
* link:https://www.oreilly.com/ideas/the-world-beyond-batch-streaming-102[The world beyond batch: Streaming 102]
* link:https://www.oreilly.com/ideas/data-architectures-for-streaming-applications[Data architectures for streaming applications]
* link:https://www.se-radio.net/2016/10/se-radio-episode-272-frances-perry-on-apache-beam/[SE-Radio Episode 272: Frances Perry on Apache Beam]
...
I developed all of this code using link:https://jupyter.org/[Jupyter] Notebooks. I've link:https://www.rittmanmead.com/blog/tag/jupyter/[written before] about how awesome notebooks are (along with link:https://jupyter.org/[Jupyter], there's link:https://zeppelin.apache.org/[Apache Zeppelin]). As well as providing a superb development environment in which both the code and the generated results can be seen, Jupyter gives the option to download a Notebook to link:https://en.wikipedia.org/wiki/Markdown[Markdown]. This blog runs on link:https://ghost.org/[Ghost], which uses Markdown as its native syntax for composing posts - so in fact what you're reading here comes directly from the notebook in which I developed the code. Pretty cool.


image::/images/2017/01/ssc01.png[]


_If you want can view the link:https://gist.github.com/rmoff/fb033086b285655ffe7f9ff0582dedbf[notebook online here], and from there download it and run it live on your own Jupyter instance_.


I used the docker image link:https://github.com/jupyter/docker-stacks/tree/master/all-spark-notebook[all-spark-notebook] to provide both Jupyter and the Spark runtime environment. By using Docker I don't have to really worry about provisioning the platform on which I want to develop the code - I can just dive straight in and start coding. As and when I'm ready to deploy the code to a 'real' execution environment (link:https://www.rittmanmead.com/blog/2016/12/etl-offload-with-spark-and-amazon-emr-part-3-running-pyspark-on-emr/[for example EMR]), _then_ I can start to worry about that. The only external aspect was an Apache Kafka cluster that I had already, with tweets from the live Twitter feed on an Apache Kafka topic imaginatively called `twitter`.
...
See the link:https://spark.apache.org/docs/latest/api/python/pyspark.streaming.html#pyspark.streaming.StreamingContext[API reference] and link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#initializing-streamingcontext[programming guide] for more details.
...
For more information see the link:https://spark.apache.org/docs/latest/streaming-kafka-0-8-integration.html[documentation].
...
The inbound stream is a link:https://spark.apache.org/docs/2.0.0/api/python/pyspark.streaming.html#pyspark.streaming.DStream[`DStream`], which supports various built-in link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#transformations-on-dstreams[transformations] such as `map` which is used here to parse the inbound messages from their native JSON format.
...
The link:https://spark.apache.org/docs/2.0.0/api/python/pyspark.streaming.html#pyspark.streaming.DStream[`DStream`] object provides native functions to count the number of messages in the batch, and to print them to the output:


* link:https://spark.apache.org/docs/2.0.0/api/python/pyspark.streaming.html#pyspark.streaming.DStream.count[`count`]
* link:https://spark.apache.org/docs/2.0.0/api/python/pyspark.streaming.html#pyspark.streaming.DStream.pprint[`pprint`]
...
Tweets come through in a JSON structure, of which you can see an link:https://gist.github.com/rmoff/3968605712f437a1f37e7be52129cade[example here]. We're going to analyse tweets by author, which is accessible in the JSON structure at `user.screen_name`.


The link:https://pythonconquerstheuniverse.wordpress.com/2011/08/29/lambda_tutorial/[`lambda`] anonymous function is used to apply the `map` to each RDD within the DStream. The result is a DStream holding just the author's screenname for each tweet in the original DStream.
...
This is because sort is not a built-in link:https://spark.apache.org/docs/2.0.0/api/python/pyspark.streaming.html#pyspark.streaming.DStream[DStream] function. Instad we use the link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#transform-operation[`transform`] function to access link:https://spark.apache.org/docs/latest/api/python/pyspark.html#pyspark.RDD.sortBy[`sortBy`] from pySpark.
...
To display just the top five authors, based on number of tweets in the batch period, we'll using the link:https://spark.apache.org/docs/2.0.2/api/python/pyspark.html#pyspark.RDD.take[`take`] function. My first attempt at this failed with:
...
Per my link:https://stackoverflow.com/questions/41483746/transformed-dstream-in-pyspark-gives-error-when-pprint-called-on-it[woes on StackOverflow] a `parallelize` is necessary to return the values into a DStream form.
...
You can see the full output from the job in link:https://gist.github.com/rmoff/fb033086b285655ffe7f9ff0582dedbf[the notebook here].
...
Image credit, and more details about window processing, link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#window-operations[here]._


The first thing to do to enable windowed processing in Spark Streaming is to launch the Streaming Context with a link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#checkpointing[checkpoint directory] configured. This is used to store information between batches if necessary, and also to recover from failures. You need to rework your code into the pattern link:https://spark.apache.org/docs/latest/streaming-programming-guide.html#how-to-configure-checkpointing[shown here]. All the code to be executed by the streaming context goes in a function - which makes it less easy to present in a step-by-step form in a notebook as I have above.
...
You can see the full output from the job in link:https://gist.github.com/rmoff/fb033086b285655ffe7f9ff0582dedbf[the notebook here]. Let's take some extracts and walk through them.
...
What I've put together is a very rudimentary example, simply to get started with the concepts. In the examples in this article I used Spark Streaming because of its native support for Python, and the link:https://www.rittmanmead.com/blog/2016/12/etl-offload-with-spark-and-amazon-emr-part-5/[previous work] I'd done with Spark. Jupyter Notebooks are a fantastic environment in which to prototype code, and for a local environment providing both Jupyter and Spark it all you can't beat the Docker image link:https://github.com/jupyter/docker-stacks/tree/master/all-spark-notebook[all-spark-notebook].


There are other stream processing frameworks and languages out there, including link:https://flink.apache.org/[Apache Flink], link:https://www.confluent.io/blog/introducing-kafka-streams-stream-processing-made-simple/[Kafka Streams], and link:https://beam.apache.org/[Apache Beam], to name but three. link:https://storm.apache.org/[Apache Storm] and link:https://samza.apache.org/[Apache Samza] are also relevant, but whilst were early to the party seem to crop up less frequently in stream processing discussions and literature nowadays.
