...
One of the challenges that we’ve faced up until now is that all servers have had to be built from scratch in the cloud. Obviously we work with development VMs on local machines too, so wouldn’t it be nice if we could build VMs locally and then push them to the cloud? Well, now we can. Amazon offer a route to import virtual machines, and in this article I’m going to show how that works. I’ll use the superb link:https://blogs.oracle.com/proactivesupportEPM/entry/obiee_sample_app_v406_is[SampleApp v406] VM that Oracle provide, because this is a great real-life example of a VM that is so useful, but many developers can find too memory-intensive to be able to run on their local machines all the time.
...
. Download the six-part archive from link:http://www.oracle.com/technetwork/middleware/bi-foundation/obiee-samples%E2%80%93167534.html[http://www.oracle.com/technetwork/middleware/bi-foundation/obiee-samples–167534.html]
...
This took 12 minutes or so, but that was on a high-spec system, so link:http://en.wiktionary.org/wiki/your_mileage_may_vary[YMMV].
...
Importing Linux VMs to Amazon EC2 will only work if the kernel is supported, which according to link:http://aws.amazon.com/blogs/aws/vm-import-export-for-linux/[an AWS blog post] includes Red Hat Enterprise Linux 5.1 - 6.5. Whilst SampleApp v406 is built on Oracle Linux 6.5 (which isn’t listed by AWS as supported), we have the option of telling the VM to use a kernel that is Red Hat Enterprise Linux compatible (instead of the default Unbreakable Enterprise Kernel - UEK). There are some other pre-requisites that you need to check if you’re trying this with your own VM, including a network adaptor configured to use DHCP. The link:http://aws.amazon.com/blogs/aws/vm-import-export-for-linux/[aforementioned blog post] has details.


. Boot the VirtualBox VM, which should land you straight in the desktop environment, logged in as the *oracle* user.
. We need to modify a file as *root* (superuser). Here’s how to do it graphically, or use `vi` if link:http://xkcd.com/378/[you’re a real programmer]:
...
_The sudo bit is important, because it tells Linux to run the command as root. (I’m on an xkcd-roll here: link:http://xkcd.com/149/[1], link:http://xkcd.com/838/[2])_
...
. link:https://portal.aws.amazon.com/gp/aws/developer/registration/index.html[An AWS account]

+
. You’ll also need your AWS account’s link:http://docs.aws.amazon.com/IAM/latest/UserGuide/ManagingCredentials.html#Using_CreateAccessKey[Access Key and Secret Key]
+
. link:http://docs.aws.amazon.com/AWSEC2/latest/CommandLineReference/set-up-ec2-cli-linux.html[AWS EC2 commandline tools installed], along with a Java Runtime Environment (JRE) 1.7 or greater:
...
Before we can do the upload we need an S3 “bucket” to put the disk image in that we’re uploading. You can create one from link:https://console.aws.amazon.com/s3/[https://console.aws.amazon.com/s3/]. In this example, I’ve got one called *rmc-vms* - but you’ll need your own.


image::/images/2014/09/420cec2_03.png[]


Once the bucket has been created, we build the command line upload statement using link:http://docs.aws.amazon.com/AWSEC2/latest/CommandLineReference/ApiReference-cmd-ImportInstance.html[ec2-import-instance]:
...
* *m3.large* is the spec for the VM. You can see the link:http://aws.amazon.com/ec2/instance-types/[available list here]. In the link:http://aws.amazon.com/blogs/aws/vm-import-export-for-linux/[AWS blog post] it suggests only a subset will work with the import method, but I’ve not hit this limitation yet.
* *region* is the link:http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html[AWS Region] in which the EBS volume and EC2 instance will be built. I’m using ew-west-1 (Ireland), and it makes sense to use the one geographically closest to where you or your users are located. Still waiting for link:http://en.wikipedia.org/wiki/Yorkshire[uk-yorks-1]...
...
At this point you can remove the VMDK from S3 (and should do, else you'll continue to be charged for it), following the link:http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/cleaning-up-after-an-upload.html[instructions for *ec2-delete-disk-image*]
...
* Running a server on AWS EC2 costs real money, so watch out. Once you’ve put your credit card details in, Amazon will continue to charge your card whilst there are chargeable items on your account (EBS volumes, instances - running or not- , and so on). You can get an idea of the link:http://calculator.s3.amazonaws.com/index.html[scale of charges here].
* As mentioned above, a server on the open internet is a lot more vulnerable than one virtualised on your local machine. You _will_ get link:http://2.bp.blogspot.com/-hCaoHA8AJys/TsKLRZHzDLI/AAAAAAAAB_c/lbwE-D9BfOM/s1600/kenneth.jpg[poked and probed], usually by automated scripts looking for open ports, weak passwords, and so on. SampleApp is designed to open the toybox of a pimped-out OBIEE deployment to you, it is not “hardened”, and you risk learning the tough way about the need for it if you’re not careful.
...
There’s a whole host of link:http://www.oracle.com/technetwork/community/developer-vm/index.html[VirtualBox “appliances” out there], and some of them such as the developer-tools-focused ones only really make sense as local VMs. But there are plenty that would benefit from a bit of “Cloud-isation”, where they’re too big or heavy to keep on your laptop all the time, but are handy to be able to spin up at will. A prime example of this for me is the EBS Vision demo database that we use for our BI Apps training. Oracle used to provide an pre-built Amazon image (know as an AMI) of this, but since withdrew it. However, Oracle do publish link:https://support.oracle.com/epmos/faces/DocContentDisplay?id=1620448.1[Oracle VM VirtualBox templates for EBS 12.1.3 and 12.2.3] (link:https://blogs.oracle.com/stevenChan/entry/oracle_vm_virtual_appliances_available[related blog]), so from this with a bit of leg-work and a big upload pipe, it's a simple matter to brew your own AWS version of it -- ready to run whenever you need it.
