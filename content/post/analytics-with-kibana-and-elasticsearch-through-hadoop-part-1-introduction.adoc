...
This is where ‘big data’ gets fun, because instead of “Acme DI” and “Acme Database” and “Acme BI”, we have the much more interesting - if somewhat link:https://www.youtube.com/watch?v=Iu7vySQbgXI[silly] - naming conventions of the whackier the better. Here I’m using:


* link:http://www.elasticsearch.org/overview/kibana[Kibana] - data visualisation tool for Elasticsearch
* link:http://www.elasticsearch.org/overview/elasticsearch[Elasticsearch] - data store & analytics / search engine
* link:http://hadoop.apache.org/docs/r0.19.0/hdfs_shell.html[HDFS] - Hadoop’s distributed file system
* link:http://www.mongodb.org/[MongoDB] - NoSQL database
* link:http://hive.apache.org/[Hive] - enables querying data held in various places including HDFS (and Elasticsearch, and MongoDB) with a SQL-like query language
* link:https://cwiki.apache.org/confluence/display/Hive/HiveServer2+Clients#HiveServer2Clients-Beeline%E2%80%93NewCommandLineShell[Beeline] - Hive command line interface
* link:http://datasift.com/[Datasift] - online service that streams tweets matching a given pattern to a nominated datastore (such as MongoDB)
* link:https://github.com/mongodb/mongo-hadoop/releases[mongo-hadoop] - a connector for MongoDB to Hadoop including Hive
* link:https://github.com/elasticsearch/elasticsearch-hadoop[elasticsearch-hadoop] - a connector for Elasticsearch to Hadoop including Hive
...
For building and testing the SerDe regex link:http://rubular.com/[Rubular] is most excellent, but note that it’s Java regex you’re specifying in the SerDe which has its differences from Python or Ruby regex that Rubular (and most other online regex testers) support. For the final validation of Java regex I use the slightly ugly but still useful link:http://www.regexplanet.com/advanced/java/index.html[regexplanet], which also gives you the fully escaped version of your regex which you’ll need to use for the actual Hive DDL/DML.
...
The other point to note is that we’re now using *link:https://github.com/mongodb/mongo-hadoop/[mongo-hadoop]* for Hive to connect to MongoDB. I found that I had to first build the full set of jar files by running `./gradlew jar -PclusterVersion='cdh5'`, and also download the link:http://central.maven.org/maven2/org/mongodb/mongo-java-driver/[MongoDB java driver], before copying the whole lot into `/usr/lib/hadoop/lib`. This is what I had by the end of it:
...
. To be a fully rounded geek, I should learn regex. _Looks up regex_. Hmm, looks complicated….link:https://www.youtube.com/watch?v=xrAIGLkSMls&feature=youtu.be&t=15s[Squirrel!]

+
. _To be a fully *round* (geddit?!) geek, I should keep eating these link:https://twitter.com/rmoff/statuses/524807504170340352[big breakfasts]_
+
...
This means, take the `ref_url` column and if you find `http://www.rittmanmead.com` then replace it with nothing, i.e. delete it. The two backslashes before each forward slash simply link:http://en.wikipedia.org/wiki/Escape_character[escape] them since a forward slash on its own has a special meaning in regex. Just to keep you on your toes - Java regex requires double backspace escaping, but all other regex (including the online parser I link to below) uses a single one.
...
. We’ll match on the year and month (*`/2014/01/`*) because they’re fixed pattern, so using `\d` to match on digits and `{x}` to match _x_ repetitions: link:http://rubular.com/r/aY9QgDis37[_(see example on Rubular.com)_]
...
Secondly, the plus symbol `+` tells regex to match at least one repetitions of the preceeding group - i.e. any character that is not a forward slash. link:http://rubular.com/r/UnG8w1DIj0[_(example)_]
...
Now all we need to do is escape it for Java regex, and stick it in the Hive `REGEXP_EXTRACT` function, specifying `1` as the capture group number to extract: link:http://rubular.com/r/GvVveUarnm[_(example)_]
...
link:https://www.youtube.com/watch?v=XOhZgAPn_CU[Which is nice], because it means we can use it as the common join in our queries. For example, to look up the title of the blog post that someone has tweeted about, and who wrote the post:
