...
Turned all the log files link:http://en.wikipedia.org/wiki/Up_to_eleven[up to 11] didn’t yield anything useful. For some reason half the time Java Host would just “ignore” the custom skin. Shutting down each node proved that in isolation the custom skin configuration on each node was definitely correct, because then the colours started working just fine. It was only when multiple Java Hosts across the nodes were active that there was a problem.
...
If you want to find out more about this kind of thing, link:http://www.brendangregg.com/linuxperf.html[this page is a great starting point].
