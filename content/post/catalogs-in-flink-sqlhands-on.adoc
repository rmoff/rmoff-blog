---
title: 'Catalogs in Flink SQL—Hands On'
date: "2024-02-19T00:00:00+00:00"
draft: false
credit: "https://bsky.app/profile/rmoff.net"
categories:
- Apache Flink
image: "/images/2024/02/flink-hands-on.webp"
original_source: "decodable"
original_url: "https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on"
---

NOTE: This post originally appeared on the link:https://www.decodable.co/blog/catalogs-in-flink-sql-hands-on[Decodable blog].

In the  link:/2024/02/16/catalogs-in-flink-sqla-primer/[previous blog post]  I looked at the role of catalogs in Flink SQL, the different types, and some of the quirks around their configuration and use.
If you are new to Flink SQL and catalogs, I would recommend reading that post just to make sure you’re not making some of the same assumptions that I mistakenly did when looking at this for the first time.

<!--more-->
In this article I am going to walk through the use of several different catalogs and go into a bit of depth with each to help you understand exactly what they’re doing—partly out of general curiosity, but also as an important basis for troubleshooting.
I’ll let you know now: some of this goes off on tangents somewhat, but I learnt lots during it and I want to share that with you!
...
And if you're using a link:https://decodable.co/[managed Flink service] , irrelevant 😄.
...
As mentioned; this is just for a local setup—if you decide to take this on to real use then you'll want to make sure you're persisting the data in a proper RDBMS.
...
This is `hive-site.xml` and needs to go in Flink’s `./conf` folder (by default):
...
Now we get to the fun bit— link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/table/hive/overview/#dependencies[dependencies] !
...
I've identified these by trial-and-error because I was offended by needing such a heavy-weight download `¯\_(ツ)_/¯`.
...
/Users/rmoff/hadoop/hadoop-3.3.2/etc/hadoop:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/common/lib/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/common/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/hdfs:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/hdfs/lib/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/hdfs/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/mapreduce/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/yarn:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/yarn/lib/*:/Users/rmoff/hadoop/hadoop-3.3.2/share/hadoop/yarn/*
----
_You'll notice I'm using $( ) instead of ` ` to enclose the hadoop call because to me it's more readable and less ambiguous—I read the docs as meaning you just had to put the Hadoop classpath in the place of hadoop classpath, not that it was an actual command to run._
...
I mentioned Catalog Stores in my  link:/2024/02/16/catalogs-in-flink-sqla-primer/[first blog post] , and I've not defined one—meaning that the catalog _definition_ is not persisted between sessions.
...
+
Of course, none of this is actually _necessary_ for simply using a catalog with Flink—but I found it useful for mapping out in my mind what's actually happening.
...
We’ll store the data on link:https://min.io/[MinIO] , which is an S3-compatible object store that you can run locally, and write it in the widely-adopted link:https://parquet.apache.org/[Apache Parquet]  column-oriented file format.
...
Which is pretty neat—it's just not what you might assume it does.
...
You also need the JDBC driver of the database to which you want to connect—the docs have a useful reference to the link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/table/jdbc/#dependencies[download links]  for these.
...
Note that Flink doesn’t use the concept of schemas so link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/table/jdbc/#jdbc-catalog-for-postgresql[as noted in the docs]  the Postgres schema (`public` in this example) is prepended to the table name shown in Flink.
...
Per link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/table/jdbc/#jdbc-catalog[the documentation] , the creation of new objects (such as tables) isn’t supported:
...
😉 I'm going to dig into the deluge of dastardly debugging in a subsequent blog—for now we'll just look at things when they go right.
...
So as above—where we use the Hive Metastore as a Flink catalog directly—I'm using the link:https://github.com/recap-build/hive-metastore-standalone[standalone Hive metastore Docker image] .
...
Therefore you need to add the S3 endpoint and authentication details to link:https://github.com/decodableco/examples/blob/main/catalogs/flink-iceberg-hive/hms-standalone-s3/conf/hive-site.xml[the hive-site.xml on the Hive Metastore too] —not just Flink:
...
└─────────┴───────┘
----
How does Flink know to go to the bucket called `warehouse` and path `db_rmoff.db/t_foo/[…]` to find the data and metadata for the table?
That's where the Catalog comes in.
The Hive metastore—in this case—holds the magical metadata of this relationship, which we can see if we query the embedded DerbyDB:
...
This permutation is obviously—given the use of DynamoDB—designed for when you're running Flink on AWS, perhaps with EMR.
...
----
This all looks good!
As you'd expect, the data and metadata written to disk is the same as above when we use the Hive Metastore—because all we're changing out here is the metastore layer, everything else is the same.
...
Iceberg actually supports link:https://iceberg.apache.org/javadoc/latest/org/apache/iceberg/BaseMetastoreCatalog.html[9 catalog types] , but don't worry—I'm not going to go through each one 😅.
...
I set the credentials for the S3 storage link:https://github.com/decodableco/examples/blob/main/catalogs/flink-iceberg-jdbc/[as environment variables] —there is probably a better way to do this.
...
== In Conclusion…
If you’ve stuck with me this far, well done!
🙂 My aim was not to put you through the same pain as I had in traversing this, but to summarise the key constants and variables when using the different components and catalogs.

Stay tuned to this blog for my link:/2024/02/27/flink-sql-and-the-joy-of-jars/[next] link:/2024/08/06/troubleshooting-flink-sql-s3-problems/[posts] which will be a look at some of the troubleshooting techniques that can be useful when exploring Flink SQL.
