...
A clear example of PaaS is the Hadoop platform. You can run a Hadoop cluster yourself, whether on Oracle's Big Data Appliance (BDA), or maybe on your own hardware (or indeed, on IaaS in the cloud) but with a distribution such as Cloudera's CDH. Point being, you still have to manage it, to tune your Hadoop parameters, and so on. Hadoop as a platform in the cloud (i.e. PaaS) is offered by many companies, including the big vendors, such as Oracle (link:https://cloud.oracle.com/bigdata?ref=rittmanmead.com[Big Data Cloud Service]), Microsoft (link:https://azure.microsoft.com/en-us/services/hdinsight/?ref=rittmanmead.com[HDInsight]), Google (link:https://cloud.google.com/dataproc/?ref=rittmanmead.com[Dataproc]) -- and then the daddy of them all, Amazon with it's link:https://aws.amazon.com/emr/?ref=rittmanmead.com[Elastic Map Reduce (EMR)] platform


Another example of PaaS is link:https://cloud.oracle.com/business_intelligence?ref=rittmanmead.com[Oracle's BI Cloud Service] (BICS), in which you build and run your own RPD and reports, but Oracle look after the actual server processes.
...
Before you shoot me down for a hyperbolic strawman argument, there is an important dichotomy to draw here, between *innovation* and *execution*. Applicable to the world of big data in general, it is a useful concept spelt out in the link:http://www.oracle.com/technetwork/database/bigdata-appliance/overview/bigdatarefarchitecture-2297765.pdf?ref=rittmanmead.com[Oracle Information Management & Big Data Reference Architecture]. For data to provide value, it doesn't have to land straight away into the world of formalised development processes, Production environments, and so on. A lot of the time you will want to 'poke around' with it, to explore it -- to *innovate*. Of the technology base out there, you may not know which tool, or library, is going to yield the best results. This is where the "discovery lab" comes in, and where the type of hand-cranked Spark coding that I've demonstrated sits:
...
ODI is the premier DI tool on the market, with link:https://docs.oracle.com/middleware/1221/odi/odi-big-data/intro.htm?ref=rittmanmead.com#ODIBD107[good support] for "big data" technologies, including the ability to generate Spark code to perform transformation. It can be deployed to run on Amazon's EMR, link:http://www.ateam-oracle.com/using-oracle-data-integrator-odi-with-amazon-elastic-mapreduce-emr/?ref=rittmanmead.com[as illustrated here], as well as link:http://www.ateam-oracle.com/odi-on-compute-cloud-service-step-by-step-installation/?ref=rittmanmead.com[Oracle's Cloud platform]. As can be seen from link:https://static.rainfocus.com/oracle/oow16/sess/1464997671877001Bm1g/ppt/CON6622%20-%20ODI%20Overview%20-%20final.pdf?ref=rittmanmead.com[this presentation] from Oracle Open World in September 2016, there's additional capabilities coming including around Kafka, Spark Streaming, and Cassandra.


image::/images/2016/12/acme13.png[]


Another route to examine, alongside ODI, is the ecosystem within AWS itself around code execution and orchestration with tools such as link:https://aws.amazon.com/lambda/?ref=rittmanmead.com[Lambda], link:https://aws.amazon.com/datapipeline/?ref=rittmanmead.com[Data Pipeline], and link:https://aws.amazon.com/swf/?ref=rittmanmead.com[Simple Workflow Service]. There's also link:https://aws.amazon.com/glue/?ref=rittmanmead.com[AWS Glue], which like Athena was announced at re:Invent 2016. This promises three key things of crucial importance here:


* A link:https://d0.awsstatic.com/screenshots/glue/screenshot-glue-step1-data-catalog.png?ref=rittmanmead.com[Data Catalog], populated automatically, and not only just supporting multiple formats and sources, but including automatic classification (e.g. "Web Log") of the data itself.
* Automatic generation of ETL code. From the link:https://d0.awsstatic.com/screenshots/glue/screenshot-glue-step2-etl-generation.png?ref=rittmanmead.com[release announcement notes] this looks like it is pySpark-based code. So the code that I put together for this exercise here, manually (and at times, painfully), could be automagically generated based on source/target and operators required. The link:https://aws.amazon.com/glue/?ref=rittmanmead.com[announcement notes] also specifically mention the inclusion of standard ETL processes such as handling bad data
* link:https://d0.awsstatic.com/screenshots/glue/screenshot-glue-step3-orchestration.png?ref=rittmanmead.com[Orchestration and management] of ETL jobs. One of my main objections above to taking 'proof of concept' pySpark code and trying to use it in a Production scenario is that you end up with a spaghetti of scripts, which are a nightmare to maintain and support. If Glue lives up to its promises, we'd pretty much get the best of all worlds - a flexible yet robust platform.
...
_You can listen to a discussion of this project, along with other topics including OBIEE, in an episode of the Drill to Detail link:http://www.drilltodetail.com/podcast/2016/12/20/drill-to-detail-ep14-christmas-new-year-special-with-special-guest-robin-moffatt?ref=rittmanmead.com[podcast here]._
