...
link:http://www.brendangregg.com/flamegraphs.html[Flame Graphs] are a visualisation that I learnt about through the excellent Linux systems performance work of link:http://www.brendangregg.com/[Brendan Gregg], and saw link:https://twitter.com/lucacanalidb[Luca Canali] talk about recently at link:http://tech14.ukoug.org/default.asp?p=11991&dlgact=shwprs&prs_prsid=9967&day_dayid=84[UKOUG Tech 14]. They’re a brilliant way of summarising extremely dense information in a way from which the main components accounting for the most time can be identified. I was recently doing some analysis for a client on their ODI batch runtime and I thought it would be a good idea to try them out. Load Plans can have complex hierarchies to them and working out which main sections account for what time can be tricky, as can following a load plan step through to a session and on to a session step and its constituent parts.
...
Creating the flame graph is simple. You just need a stack trace that is semi-colon separated, followed by a space-delimited counter value at the end. A bit of link:http://docs.oracle.com/database/121/SQLRF/queries003.htm#SQLRF52332[recursive SQL magic] with the SNP_ tables (helpfully link:https://support.oracle.com/epmos/faces/DocContentDisplay?id=1903225.1[documented by Oracle here]) gives us this kind of output file with one line for every task executed and its duration:
...
which you then run through the link:https://github.com/brendangregg/FlameGraph[Flame Graph tool]:
...
Simply load the resulting SVG into a web browser such as Chrome, and you’re done. link:https://s3.amazonaws.com/rmc_docs/session-step-task-flame.svg[Here’s an example that you can download and try out].
