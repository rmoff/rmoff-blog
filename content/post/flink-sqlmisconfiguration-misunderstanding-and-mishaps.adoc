---
title: 'Flink SQL—Misconfiguration, Misunderstanding, and Mishaps'
...
original_url: "https://www.decodable.co/blog/flink-sql-misconfiguration-misunderstanding-and-mishaps"
---
...
I had a whole blog series about Flink SQL lined up…and then I started to write it and realised rapidly that one's initial exposure to Flink and Flink SQL can be somewhat, shall we say, _interesting_.
...
<!--more-->
Quite a few of the problems are a subset of the broader challenge of getting the right JAR in the right place at the right time—a topic link:/2024/02/27/flink-sql-and-the-joy-of-jars/[worth its own blog] .

Let's start off by looking at the JDBC connector.
This provides its own catalog, which I link:/2024/02/19/catalogs-in-flink-sqlhands-on/[explored recently]  in another post.
In that article I trod the happy path; below are the potholes and pitfalls that befell me on the way 🧌.


== The JDBC Catalog
_You can learn more about the JDBC Catalog, and Flink SQL Catalogs in general,_ link:/2024/02/16/catalogs-in-flink-sqla-primer/[here] _and_ link:/2024/02/19/catalogs-in-flink-sqlhands-on/[here] _._
...
What does Illegal Argument— the argument that we've specified for `base-url` is "Illegal" because it needs the fully qualified JDBC URL prefix.
Let's try again:
...
Now we'll do the same as above—create a JDBC catalog.
Note that this time the `base-url` _is_ referencing `localhost` because Flink is running locally on the machine and I’ve exposed Postgres’ port from its container, so `localhost` is the right place to access it.
...
Flink SQL> SELECT * FROM country_flag LIMIT 5;
----
At this point there's no error… but no nothing either.
...
----
This puzzles me, because Postgres is up—and we can verify the connectivity from SQL Client with`SHOW TABLES`:
...
I wrote link:/2024/02/27/flink-sql-and-the-joy-of-jars/[a whole article]  about JAR files because they are so central to the successful operation of Flink.
...
----
This one I'd come across before - I needed the Hadoop JARs (discussed in more detail link:/2024/02/19/catalogs-in-flink-sqlhands-on/[here] ).
I downloaded Hadoop (words I never thought I'd write in 2024 😉) and set the environment variable:
...
Not only has the `collect` job (the`SELECT` ) failed—but also the`INSERT`.
Above, I saw this:
...
Oh, foolish and naïve one am I!
This is an _asynchronous_ job submission.
The only success was that the job was submitted to the job manager to execute.
If we dig into the job in the UI (or indeed the log files for the `taskexecutor` process and `standalonesession` process and scroll up from the other error), we'll see why the `INSERT` failed—for a similar reason as the`SELECT`:
...
Well we've happily made the Parquet JAR available to the SQL Client… yet the actual business of writing and reading data is not done by the client, but the Flink task manager—and we didn't put the JAR there.
...
I used it as the basis for link:/2024/02/19/catalogs-in-flink-sqlhands-on/[exploring catalogs provided by the Iceberg Flink connector] , including using JDBC as a backing store for the catalog.
...
In fact, even more puzzling was that the catalog and table creation _had_ worked—so the connection to Postgres for those statements must have been ok.
...
java.sql.SQLException: No suitable driver found for jdbc:postgresql://postgres:5432/world-db?user=world&password=world123
----
This makes sense when we realise that the SQL Client is running using the same image as the Flink cluster—where we saw the error above.
...
1 row in set
----
It's looking promising (the job shows`FINISHED`)…
...
Same 1.18.1 environment as above when things didn't work, except with `flink-sql-connector-hive-3.1.3_2.12-1.18.1.jar` in the place of `flink-sql-connector-hive-2.3.9_2.12-1.18.1.jar` and…it works.
...
I have no idea why.
But at least I know now how to fix it 🙂
...
----
This makes sense—we're trying to use S3 (well, MinIO) but we've not provided any S3 credentials (`NoAuthWithAWSException: No AWS Credentials provided`).
The docs for S3 offer one option— link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/filesystems/s3/#configure-access-credentials[adding them to flink-conf.yaml] .
...
----
This is because we're trying to use the credentials that we've configured for MinIO (the super-secret `admin/password` 😉) against AWS S3 itself.
...
This brings together JARs, Flink configuration—and the importance of understanding _what is running where._

To set the scene: I was doing the same as the above section—I was setting up Flink 1.18 writing files to MinIO (S3 compatible storage), using Hive Metastore for catalog persistence.
...
java.net.ConnectException: Connection refused (Connection refused)
----
The `Connection refused` here is coming from the fact that the SQL Client is trying to reach the Hive MetaStore (HMS) using`thrift://localhost:9083` —but `localhost` won't work as that's local to the SQL Client container.
...
Based on my exploration of Flink SQL and JARs link:/2024/02/27/flink-sql-and-the-joy-of-jars/[previously]  I thought I'd be well-equipped to deal with this one.`ClassNotFound`?
Piece of cake.
Right?


image::/images/2024/04/6717dc5182a396e17c57bebf_6702c780e3a93018db085b11_661dde082c30ef9fef7f4481_esoOiCFdgsHC0D4l5WcNJ--KPTnsHbg_wOs1jwhOBWuRnTTRDUoYeqaSbFaq7XkroKOfugn9pOKVlj4fRnKdoNpqlSI7p7qf4RHlKvn8rExW5wI3BLBNcvf_7uBGnT9IG-6J5vyf3Oi-qVSKCvj7RmA.webp[Flink SQL—Misconfiguration, Misunderstanding, and Mishaps.png]
All we need to do is make sure that the Hadoop AWS JAR—that provides the `S3AFileSystem` class—is present.
...
[…]
----
Using `jinfo`  link:/2024/02/27/flink-sql-and-the-joy-of-jars/[we can see that]  the Classpath for the SQL Client shows that the `hadoop-aws` JAR is present: +
...
We can even double-check that this is indeed the JAR that we want by link:/2024/02/27/flink-sql-and-the-joy-of-jars/[searching its contents]  for the class:
...
Various suggestions from colleagues, Slack groups, Stack Overflow, chatGPT—and of course Google, which included:
...
To simplify things, I'm going to move the S3 config away from `FLINK_PROPERTIES` and specify it in just one place, the `./conf/hive-site.xml` on the Flink containers, where it should get used by both the jobmanager, taskmanager—and SQL Client.
...
For us that means local to the Flink container, which it's not—and is where the JAR problem comes in.

This didn't happen when we created the table as part of the catalog because the `CREATE CATALOG` included `'uri'='thrift://hms:9083'`  and thus could find the Hive Metastore.
So the lesson here is that the `uri` _must_ be specified somewhere—either in the DDL (the successful `CREATE TABLE iceberg_test` above does this), or by adding it to the`hive-site.xml`:
