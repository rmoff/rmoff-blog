...
date: "2011-01-08T18:12:05+0000"
image: "/images/2011/01/snag-2011-01-07-14-47-22-0000.webp"
categories:
- bug
- DWH
- MViews
...
2. When we load a new day’s data, for an *existing* week : truncate the partition on the weekly MV, and reload it (As any fule kno, truncate/insert is going to perform better than delete/insert)- When existing data is updated on the base table: corresponding weekly MV partition truncated and reloaded
...
- In the MView definition:
  ...
  - Alternatively partition MView on same partition key as detail table3. Define the MView **USING NO INDEX** to prevent the default global index being created
...
[See here for the documentation regarding PCT partition truncation.](http://download.oracle.com/docs/cd/B28359_01/server.111/b28313/advmv.htm#i1009128)  
...
1. **The materialized view is partitioned on the partitioning key column or join dependent expressions of the detail table.**  
   ...
   - An example of the former (partitioned on the partitioning key of the detail table) is base table and aggregate MV both partitioned on Store, or Warehouse, etc.
     ...
     ![](/images/2011/01/snag-2011-01-07-14-47-22-0000.webp "SNAG-2011-01-07-14.47.22-0000")
   ...
   - An example of the latter (join dependent expression) is where you link to a second table in the MV definition, giving Oracle the understanding of the relation between two different partitioning columns (for example, how do weeks relate to days):
     ...
     ![](/images/2011/01/snag-2011-01-07-14-48-38-0000.webp "SNAG-2011-01-07-14.48.38-0000")

     Join Dependent Expression- **The materialized view should be range or list partitioned**
