...
Another one that has bitten me twice over time – and that eludes the troubleshooting I’ll demonstrate in a moment – is that link:http://bugs.centos.org/print_bug_page.php?bug_id=4959&ref=rittmanmead.com[SELinux gets stroppy about root access using ssh keys]. I always just take this as a handy reminder to disable selinux (in `/etc/selinux/config`, set `SELINUX=disabled`), having never had cause to leave it enabled. But, if you do need it enabled you’ll need to hit the interwebs to check the exact cause/solution for this problem.
...
I'm interested in what you think – what particular tools or tips do you have for working with a cluster of Linux machines? Leave your answers in the comments below, or tweet them to me at link:https://twitter.com/rmoff/?ref=rittmanmead.com[@rmoff].
