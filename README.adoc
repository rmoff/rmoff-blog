...
First, build the custom Hugo Docker image:
...
docker run --rm -it -v $(pwd):/src -v /tmp/hugo_public:/tmp/public rmoff-blog-hugo:0.152.2 --buildFuture --buildDrafts -d /tmp/public && \
